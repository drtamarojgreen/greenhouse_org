(function () {
    /**
     * TypeScript Live Harness
     * ------------------------
     * This harness is identical in behaviour to js_test_models.js, except that it
     * loads the migrated *.ts sources directly and transpiles them in the browser
     * using the TypeScript compiler (loaded from a CDN in test_models_ts.html).
     * No compiled *.js model files are required.
     */
    const TS_LOG = (...a) => console.log('[TS-Harness]', ...a);

    if (!window.ts || typeof window.ts.transpileModule !== 'function') {
        console.error('[TS-Harness] TypeScript compiler (window.ts) is not available. ' +
            'Ensure the typescript CDN script loaded before this harness.');
    }

    // Transpile a TypeScript source string to a browser-runnable classic script.
    // Modules communicate exclusively via window globals, so we provide dummy
    // `exports`/`module` bindings to absorb the CommonJS boilerplate that the
    // compiler emits for files that use `export`.
    function transpileTS(source, fileName) {
        const out = window.ts.transpileModule(source, {
            compilerOptions: {
        ...
        return `(function(){ "use strict"; var exports = {}; var module = { exports: exports };\n${out.outputText}\n})();`;
    }

    // Map any *.js reference used by the legacy maps to its migrated *.ts sibling.
    const toTsName = (name) => String(name).replace(/\.js$/i, '.ts');

    // Fetch a source file, preferring the *.ts version and falling back to *.js
    // for files that have not (yet) been migrated (shared infrastructure, etc.).
    // Returns { code, wasTs }.
    function fetchSourceSync(tsUrl, jsUrl) {
        const read = (url) => {
        ...
        const tsCode = read(tsUrl);
        if (tsCode !== null) return { code: transpileTS(tsCode, tsUrl), wasTs: true };
        if (jsUrl) {
            const jsCode = read(jsUrl);
            if (jsCode !== null) return { code: jsCode, wasTs: false };
        }
        throw new Error(`[TS-Harness] Could not load ${tsUrl}` + (jsUrl ? ` or ${jsUrl}` : ''));
    }
    ...
    // Override or provide loadScript to load & transpile the *.ts sub-scripts.
    window.GreenhouseUtils.loadScript = async function (name, baseUrl, attributes = {}) {
        console.log(`[TS-Harness] Loading sub-script (as TS): ${name}`);
        ...
        const tsUrl = `${baseUrl}js/${toTsName(name)}`;
        const jsUrl = `${baseUrl}js/${name}`;

        return new Promise((resolve, reject) => {
            let loaded;
            try {
                loaded = fetchSourceSync(tsUrl, jsUrl);
            } catch (e) {
                return reject(e);
            }
            ...
            script.textContent = loaded.code;
            try {
                document.body.appendChild(script);
                console.log(`[TS-Harness] Sub-script executed: ${name} (${loaded.wasTs ? 'TS' : 'JS-fallback'})`);
                resolve();
            } catch (err) {
                reject(err);
            }
    ...
    async function loadModel(modelId) {
        ...
        try {
            ...
            // Fetch & transpile the migrated *.ts entry module, then execute inline.
            const tsUrl = `${baseUrl}js/${toTsName(model.js)}`;
            const jsUrl = `${baseUrl}js/${model.js}`;
            const loaded = fetchSourceSync(tsUrl, jsUrl);
            TS_LOG(`Entry module for '${modelId}' loaded (${loaded.wasTs ? 'TS' : 'JS-fallback'})`);
            script.textContent = loaded.code;
    ...
    async function executeTestBatch(testFiles, modelId = null) {
        ...
        try {
            ...
            const readTextSync = (path) => {
                ...
                // Prefer the migrated *.ts source (transpiled on the fly); fall
                // back to *.js for shared infrastructure that isn't migrated.
                const tsPath = cleanPath.replace(/\.js$/i, '.ts');
                const readRaw = (p) => {
                ...
                const tsSrc = readRaw(tsPath);
                if (tsSrc !== null) return transpileTS(tsSrc, tsPath);
                const jsSrc = readRaw(cleanPath);
                if (jsSrc !== null) return jsSrc;
                throw new Error(`Mock FS read failed for ${tsPath} / ${cleanPath}`);
            };
            ...
            // Load Tests (migrated *.ts, transpiled in-browser)
            for (const file of testFiles) {
                try {
                    const tsFile = toTsName(file);
                    let resp = await fetch(`../tests/unit/${tsFile}`);
                    let usedTs = resp.ok;
                    if (!resp.ok) {
                        // Fall back to *.js for any not-yet-migrated test file.
                        resp = await fetch(`../tests/unit/${file}`);
                    }
                    if (!resp.ok) {
                        hError(`[TS-Harness] Failed to fetch test file: ${tsFile} / ${file} (HTTP ${resp.status})`);
                        continue;
                    }
                    const raw = await resp.text();
                    const code = usedTs ? transpileTS(raw, tsFile) : raw;
                    const s = document.createElement('script');
