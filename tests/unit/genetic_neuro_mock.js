...
function setupGeneticNeuroMocks() {
    ...
    const configMockFactory = (baseData = {}) => {
        const mock = Object.assign(Object.assign({}, baseData), { get: universalGet, set: function (path, value) {
                const keys = path.split('.');
                let target = this;
        ...
        Object.defineProperty(global, name, {
            ...
            set: (v) => {
                if (v && typeof v === 'object' && v !== internalData) {
                    ...
                    if (typeof internalData.get !== 'function')
                        internalData.get = universalGet.bind(internalData);
                    if (typeof internalData.set !== 'function')
                        internalData.set = dummy;
                    if (name === 'GreenhouseGeneticConfig' && typeof internalData.import !== 'function')
                        internalData.import = dummy;
                }
            },
    ...
    if (!global.NeuroGA) {
        global.NeuroGA = class {
            constructor() { this.init = dummy; this.step = dummy; this.evaluateFitness = dummy; this.createRandomGenome = () => ({}); }
        };
        if (win !== global)
            win.NeuroGA = global.NeuroGA;
    }
}
module.exports = {
