...
TestFramework.describe('ACTUAL BUG: Mouse Control Issues', () => {
    ...
    TestFramework.describe('BUG #1: Main view uses this.camera, PiPs use separate cameras', () => {
        TestFramework.it('CRITICAL: Main view camera is NOT controlled by mainCameraController', () => {
        ...
        TestFramework.it('BUG: Main view in render() uses this.camera directly, not mainCameraController.camera', () => {
            // In render(), line 545:
    ...
    TestFramework.describe('BUG #2: PiP cameras use getState() which returns NEW object', () => {
        TestFramework.it('CRITICAL: getState() returns a NEW object, not the camera reference', () => {
        ...
        TestFramework.it('BUG: PiP views use cameraState properties, not cameraState.camera', () => {
    ...
    TestFramework.describe('BUG #3: Main view also reconstructs camera in drawTargetView', () => {
        TestFramework.it('CRITICAL: Main view passes { camera: this.camera } which creates wrapper object', () => {
            // In render(), line 545:
            // this.drawTargetView(ctx, 0, 0, w, h, activeGene, { camera: this.camera });
            //                                                    ^^^^^^^^^^^^^^^^^^^^^^
    ...
    TestFramework.describe('BUG #4: Mouse events modify controller cameras, but render uses reconstructed cameras', () => {
        TestFramework.it('REPRODUCE BUG: Drag PiP, camera changes, but render uses old values', () => {
            // 1. Get initial state
        ...
        TestFramework.it('ACTUAL BUG: Check if getState() returns correct values', () => {
    ...
    TestFramework.describe('BUG #5: Auto-rotate in drawDNAHelixPiP overrides user rotation', () => {
        TestFramework.it('CRITICAL BUG FOUND: Auto-rotate adds to rotationY every frame!', () => {
        ...
        TestFramework.it('BUG: Auto-rotate should be disabled when user interacts', () => {
    ...
    TestFramework.describe('BUG #6: Main view and PiP target view both use this.camera', () => {
        TestFramework.it('CRITICAL: Main view and PiP target view share the same camera!', () => {
            // In render(), line 545:
            // this.drawTargetView(ctx, 0, 0, w, h, activeGene, { camera: this.camera });
            //                                                              ^^^^^^^^^^^
    ...
    TestFramework.describe('ROOT CAUSE ANALYSIS', () => {
        TestFramework.it('ISSUE: drawDNAHelixPiP always adds auto-rotate, ignoring controller state', () => {
        ...
        TestFramework.it('SOLUTION: Remove auto-rotate from drawDNAHelixPiP, let controller handle it', () => {
        ...
        TestFramework.it('ISSUE #2: Other draw functions may also reconstruct camera incorrectly', () => {
            // We need to check:
