...
private:
    ...
    void serialize_value(std::vector<uint8_t>& data) const noexcept {
        if constexpr (std::is_same_v<T, std::string>) {
            ...
            append_bytes(data, &str_len, sizeof(str_len));
            append_bytes(data, value.data(), str_len);
    ...
    void deserialize_value(const uint8_t* data, size_t len,
        ...
        if constexpr (std::is_same_v<T, std::string>) {
            if (offset + sizeof(uint32_t) <= len) {
                uint32_t str_len;
                std::memcpy(&str_len, data + offset, sizeof(str_len));
                offset += sizeof(str_len);
                if (offset + str_len <= len) {
                    value.assign(reinterpret_cast<const char*>(data + offset), str_len);
                }
            }
        } else if constexpr (std::is_trivially_copyable_v<T>) {
