...
static ErrorNodeBase* deserialize_error(const uint8_t* data, size_t len,
    ...
    switch (type_id) {
        ...
        case 0: { // std::string
            auto wrapper = ErrorWrapper<std::string>::deserialize(data, len);
            return new ErrorNode<std::string>(wrapper);
        }
...
static void receiver_thread_func(RpcProtocol& rpc, AnalyzerQueue& queue,
                            Benchmark& bench) {
...
static std::string format_timestamp(uint64_t ts) {
    ...
    char buf[32];
    std::snprintf(buf, sizeof(buf), "%012" PRIu64, static_cast<uint64_t>(ts % 1000000000000ULL));
...
int main(int argc, char* argv[]) {
    ...
    RpcProtocol rpc;
    if (!rpc.init_analyzer(session_id)) {
        std::fprintf(stderr, "Failed to connect to host (session %u). "
    ...
    std::thread receiver(receiver_thread_func, std::ref(rpc), std::ref(queue),
                         std::ref(bench));
