    ...
    const GreenhouseStressPathway = {
        ...
        render(ctx, state, camera, projection, ui3d) {
            ...
            if (GraphViewer && (!GraphViewer.graphData) && window.GreenhouseGraphParser) {
                if (typeof GraphViewer.initDataOnly === 'function')
                    GraphViewer.initDataOnly();
            ...
            const nodes = sourceNodes.map(node => {
                ...
                if (node.id === 'hypothalamus' || node.type === 'gene')
                    throb = 1 + Math.sin(time * 0.005) * 0.08 * car;
                return Object.assign(Object.assign(Object.assign({}, node), p), { throb });
            });
        ...
        drawBursts(ctx) {
            ...
            this.bursts.forEach(b => {
                ...
                if (b.type === 'neural') {
                    ...
                    ctx.beginPath();
                    ctx.moveTo(bx, by);
                    ctx.lineTo(bx - (b.tx - b.x) * 0.1, by - (b.ty - b.y) * 0.1);
                    ctx.stroke();
                }
                else {
