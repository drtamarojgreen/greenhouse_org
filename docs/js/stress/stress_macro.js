...
(function () {
    ...
    const GreenhouseStressMacro = {
        ...
        drawMentalHealthFactors(ctx, state, camera, projection, ui3d) {
            ...
            activeFactors.forEach((fid, idx) => {
                ...
                if (p.scale > 0) {
                    ...
                    if (fid.includes('sleep') || fid.includes('work'))
                        target = ui3d.brainShell.regions.pfc.centroid || target;
                    else if (fid.includes('noise') || fid.includes('serotonin'))
                        target = ui3d.brainShell.regions.amygdala.centroid || target;
                    ...
                    if (fid.includes('sleep'))
                        ctx.setLineDash([10, 10]);
                    else if (fid.includes('noise'))
                        ctx.setLineDash([2, 2]);
                    else
                    ...
                    ctx.beginPath();
                    ctx.arc(px, py, 4 * p.scale, 0, Math.PI * 2);
                    ctx.fill();
                    ...
                    ctx.shadowColor = '#fff';
                    ctx.beginPath();
                    ctx.arc(p.x, p.y, (isHovered ? 8 : 5) * p.scale, 0, Math.PI * 2);
                    ctx.fill();
                    ctx.shadowBlur = 0;
                    const t = (k) => (window.GreenhouseModelsUtil && window.GreenhouseModelsUtil.t) ? window.GreenhouseModelsUtil.t(k) : k;
        ...
        drawEliteBrain(ctx, shell, camera, projection, stressIntensity, ui3d) {
            ...
            const len = Math.sqrt(Math.pow(lightDir.x, 2) + Math.pow(lightDir.y, 2) + Math.pow(lightDir.z, 2));
            lightDir.x /= len;
            lightDir.y /= len;
            lightDir.z /= len;
            ...
            faces.forEach(f => {
                ...
                if (isHovered) {
                    ...
                    if (Math.random() < vagalTone * 0.1)
                        color = 'rgba(255, 255, 255, 0.8)'; // Nerve firing spark
                }
                ctx.fillStyle = color;
                ctx.beginPath();
            ...
            ctx.lineTo(projection.width, scanPos);
            ctx.stroke();
            ...
            for (const key in ui3d.brainShell.regions) {
                ...
                if (region.centroid && key !== 'cortex' && key !== 'brainstem') {
                    ...
                    if (p.scale > 0 && dot > 0.3) {
                        ...
                        if (isVagus)
                            intensity = state.metrics.vagalTone || 0.5;
                        // Technical HUD Line
                        ctx.beginPath();
        ...
        drawProbeWave(ctx, x, y, width, intensity, alpha, key) {
            if (!this.waveOffsets[key])
                this.waveOffsets[key] = Math.random() * 100;
        ...
        drawContextualZoom(ctx, ui3d, state) {
            const hovered = ui3d.app.ui.hoveredElement;
            if (!hovered || hovered.type !== 'checkbox')
                return;
            const w = 280, h = 160;
            ...
            ctx.lineWidth = 1;
            if (ui3d.app.roundRect)
                ui3d.app.roundRect(ctx, x, y, w, h, 15, true, true);
            // Header
            ctx.fillStyle = '#A0AEC0';
            ctx.font = 'bold 9px monospace';
