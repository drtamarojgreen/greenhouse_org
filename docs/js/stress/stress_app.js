    ...
    const GreenhouseStressApp = {
        ...
        init(selector, baseUrl = '') {
            ...
            if (window.GreenhouseStressUI3D)
                window.GreenhouseStressUI3D.init(this);
        ...
        handleMouseDown(e) {
            ...
            if (Math.round(this.engine.state.factors.viewMode) === 1 && this.ui.pathwayButtons) {
                for (const b of this.ui.pathwayButtons) {
                    if (mx >= b.x && mx <= b.x + b.w && my >= b.y && my <= b.y + b.h) {
                        this.engine.state.factors.activePathway = b.pathwayId;
                        return;
                    }
                }
            }
            ...
            if (window.GreenhouseStressUI3D) {
                ...
                if (hit3D && hit3D.type === 'category_node') {
                    ...
                    if (this.ui.lockedTooltip && this.ui.lockedTooltip.id === hit3D.id) {
                        ...
                        this.ui.lockedTooltip = Object.assign(Object.assign({}, hit3D), { x: mx, y: my });
                    }
                    return;
                }
            }
        ...
        handleMouseMove(e) {
            ...
            if (!this.ui.hoveredElement && this.ui.pathwayButtons) {
                for (const b of this.ui.pathwayButtons) {
                    if (mx >= b.x && mx <= b.x + b.w && my >= b.y && my <= b.y + b.h) {
                        this.ui.hoveredElement = Object.assign(Object.assign({}, b), { type: 'pathway_button' });
                        break;
                    }
                }
            }
            ...
            if (!this.ui.hoveredElement && window.GreenhouseStressPathwayButtons) {
                ...
                for (const b of buttons) {
                    if (mx >= b.x && mx <= b.x + b.w && my >= b.y && my <= b.y + b.h) {
                        this.ui.hoveredElement = Object.assign(Object.assign({}, b), { type: 'button_overlay' });
                        break;
                    }
                }
            }
        ...
        updateModel(state, dt) {
            ...
            if (m.allostaticLoad > 0.7 && f.stress_system_stepped_care)
                clinicalDamping *= 1.25;
            // Multimodal Synergy (Item 79)
            let synergy = 1.0;
            if (scores.interv > 0 && scores.therapy > 0 && scores.lifestyle > 0)
                synergy = 1.35;
            else if ((scores.interv > 0 && scores.therapy > 0) || (scores.therapy > 0 && scores.lifestyle > 0))
                synergy = 1.15;
            ...
            const drift = 0.0002;
            m.allostaticLoad = Util.SimulationEngine.clamp(m.allostaticLoad + (m.autonomicBalance * 0.0015) + drift - (damping * 0.001), 0.05, 1.0);
            ...
            m.resilienceReserve = Util.SimulationEngine.clamp(m.resilienceReserve - (m.allostaticLoad * 0.002) + recovery + ((f.psych_support || 0) * 0.0005), 0, 1.0);
            ...
            if (f.bio_hrv)
                m.hrv = Util.SimulationEngine.smooth(m.hrv, m.hrv + 15, 0.05);
            if (f.bio_il6 || f.bio_tnf || f.bio_crp)
                m.allostaticLoad += 0.0005;
            if (f.bio_bdnf)
                m.resilienceReserve += 0.0002;
            if (f.bio_oxy)
                m.autonomicBalance = Util.SimulationEngine.smooth(m.autonomicBalance, m.autonomicBalance * 0.8, 0.02);
            // Final safety clamp
        ...
        render() {
            ...
            if (window.GreenhouseStressUI3D)
                window.GreenhouseStressUI3D.render(ctx, state, this.camera, this.projection);
            this.drawUI(ctx, w, h, state);
        },
        drawUI(ctx, w, h, state) {
            ctx.fillStyle = '#fff';
            ctx.font = 'bold 22px Quicksand, sans-serif';
            ctx.fillText(t('stress_ui_engine_title'), 40, 40);
            ...
            mLabels.forEach((ml, i) => {
                ...
                ctx.fillStyle = ml.c;
                ctx.fillRect(bx, h - 80, 2, 50);
                ctx.fillStyle = 'rgba(255,255,255,0.5)';
                ctx.font = '9px Quicksand, sans-serif';
                ctx.fillText(ml.l, bx + 10, h - 65);
                ctx.fillStyle = '#fff';
                ctx.font = 'bold 13px Quicksand, sans-serif';
                ctx.fillText(ml.v, bx + 10, h - 45);
            });
