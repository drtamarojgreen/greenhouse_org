...
(function () {
    ...
    G.Sidebar = {
        render(container, config, currentLanguage, callbacks) {
            ...
            const categories = [
                { id: 'preSynapticTerminal', label: t('Pre-Synaptic Terminal'), color: '#707870' },
                { id: 'postSynapticTerminal', label: t('Post-Synaptic Terminal'), color: '#2c3e50' },
                { id: 'vesicle', label: t('label_vesicle'), color: config.accentGold },
                { id: 'neurotransmitter', label: t('label_neurotransmitter'), color: config.accentCyan },
            ];
            ...
            let html = `
                <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 30px;">
                    ...
                    <h2 style="font-family: 'Quicksand', sans-serif; font-size: 28px; margin: 0; color: #fff;">${t('Synapse Model')}</h2>
                </div>
                ...
                <div style="margin-bottom: 25px; padding: 20px; background: rgba(255,255,255,0.03); border-radius: 15px; border: 1px solid rgba(255,255,255,0.05);">
                    <label for="scenario-selector" style="display: block; font-size: 11px; text-transform: uppercase; letter-spacing: 1px; color: #357438; margin-bottom: 8px; font-weight: 700;">${t('Research Scenario')}</label>
                    <select id="scenario-selector" aria-label="${t('Select Research Scenario')}" style="width: 100%; background: #1a1c1e; color: #fff; border: 1px solid rgba(255,255,255,0.1); padding: 10px; border-radius: 8px; font-family: inherit; font-size: 14px; cursor: pointer; margin-bottom: 15px;">
                        ${scenarioOptions}
                    </select>

                    <label for="nt-selector" style="display: block; font-size: 11px; text-transform: uppercase; letter-spacing: 1px; color: #357438; margin-bottom: 8px; font-weight: 700;">${t('Select Neurotransmitter')}</label>
                ...
                <h3 style="font-size: 12px; text-transform: uppercase; letter-spacing: 2px; opacity: 0.5; margin-bottom: 20px;">${t('Neural Anatomy')}</h3>
            ...
            html += `
                ...
                <div style="margin-top: 30px; padding: 20px; background: rgba(53, 116, 56, 0.1); border-radius: 15px; border: 1px solid rgba(53, 116, 56, 0.2);">
                    <p style="font-size: 11px; color: #357438; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 8px; font-weight: 700;">${t('Mechanics & Pathophysiology')}</p>
