...
(function () {
    ...
    G.updateMolecular = function () {
        ...
        for (let i = mState.rgsProteins.visual.length - 1; i >= 0; i--) {
            ...
            if (rgs.life <= 0)
                mState.rgsProteins.visual.splice(i, 1);
        }
        ...
        for (let i = mState.campMicrodomains.length - 1; i >= 0; i--) {
            ...
            if (m.life <= 0)
                mState.campMicrodomains.splice(i, 1);
        }
        ...
        for (let i = mState.pka.subunits.length - 1; i >= 0; i--) {
            ...
            if (s.life <= 0)
                mState.pka.subunits.splice(i, 1);
        }
        ...
        for (let i = mState.erkPathway.visual.length - 1; i >= 0; i--) {
            ...
            v.y += 0.3;
            if (v.life <= 0)
                mState.erkPathway.visual.splice(i, 1);
        }
        // 65. CREB Activation
        ...
        for (let i = mState.internalizedReceptors.length - 1; i >= 0; i--) {
            ...
            if (r.life <= 0)
                mState.internalizedReceptors.splice(i, 1);
        }
    ...
    G.renderMolecular = function (ctx, project) {
        ...
        mState.gProteins.forEach(gp => {
            ...
            if (p.scale > 0) {
                ...
                if (gp.subunit === 'alpha' && gp.gtpBound)
                    ctx.fillStyle = gp.type === 'Gs' ? '#E0E0E0' : '#D0D0D0';
