...
/**
 * Interface representing a 3D connection between neurons.
 */
...
/**
 * Interface representing a 3D particle.
 */
...
/**
 * Interface representing a 3D brain shell.
 */
interface BrainShell3D {
...
(function() {
    ...
    const GreenhouseModelsUI3D = {
        ...
        context3D: null as CanvasRenderingContext2D | null,
        camera: {
            x: 0,
            y: 0,
            z: -500,
            ...
            rotationZ: 0,
            fov: 500
        } as Greenhouse.Camera,
        projection: {
            width: 800,
            height: 600,
            near: 10,
            far: 2000
        } as Greenhouse.Projection,
        isActive: false,
        autoRotate: false,
        rotationSpeed: 0.005,
        neurons3D: [] as Neuron3D[],
        connections3D: [] as Connection3D[],
        particles: [] as Particle3D[],
        animationFrame: null as number | null,
        brainShell: null as BrainShell3D | null,
        hoveredRegion: null as string | null,
        state: (window as any)._greenhouseModelsAttributes || {} as any,
        util: (window as any).GreenhouseModelsUtil || {} as any,
        ...
        init3DCanvas() {
            ...
            canvas3DTitle.textContent = this.util.t ? this.util.t('3d_view_title') : '3D Neural Network View';
         ...
         */
        setup3DControls(container: HTMLElement) {
            const t = (k: string) => this.util.t ? this.util.t(k) : k;
            ...
            const rotateBtn = document.getElementById('rotate-3d-btn') as HTMLButtonElement;
            const resetBtn = document.getElementById('reset-camera-btn') as HTMLButtonElement;

            rotateBtn.addEventListener('click', () => this.toggleAutoRotate());
            resetBtn.addEventListener('click', () => this.resetCamera());

            // Camera control sliders
            (document.getElementById('camera-x-slider') as HTMLInputElement).addEventListener('input', (e) => {
                const target = e.target as HTMLInputElement;
                this.camera.rotationX = (window as any).GreenhouseModels3DMath.degToRad(parseFloat(target.value));
                this.render3DView();
            });

            (document.getElementById('camera-y-slider') as HTMLInputElement).addEventListener('input', (e) => {
                const target = e.target as HTMLInputElement;
                this.camera.rotationY = (window as any).GreenhouseModels3DMath.degToRad(parseFloat(target.value));
                this.render3DView();
            });

            (document.getElementById('camera-z-slider') as HTMLInputElement).addEventListener('input', (e) => {
                const target = e.target as HTMLInputElement;
                this.camera.z = parseFloat(target.value);
                this.render3DView();
            });

            (document.getElementById('fov-slider') as HTMLInputElement).addEventListener('input', (e) => {
                const target = e.target as HTMLInputElement;
                this.camera.fov = parseFloat(target.value);
                this.render3DView();
            });
        },
        ...
        setup3DInteraction() {
            ...
            this.canvas3D.addEventListener('mousedown', (e) => {
                ...
                lastX = e.clientX;
                lastY = e.clientY;
                if (this.canvas3D) this.canvas3D.style.cursor = 'grabbing';
            });
            ...
            this.canvas3D.addEventListener('mousemove', (e) => {
                ...
                if (this.camera.rotationY !== undefined) this.camera.rotationY += deltaX * 0.005;
                if (this.camera.rotationX !== undefined) this.camera.rotationX += deltaY * 0.005;

                // Update sliders
                const sliderX = document.getElementById('camera-x-slider') as HTMLInputElement;
                const sliderY = document.getElementById('camera-y-slider') as HTMLInputElement;
                if (sliderX) sliderX.value = String((window as any).GreenhouseModels3DMath.radToDeg(this.camera.rotationX || 0));
                if (sliderY) sliderY.value = String((window as any).GreenhouseModels3DMath.radToDeg(this.camera.rotationY || 0));
            ...
            this.canvas3D.addEventListener('wheel', (e) => {
                ...
                const sliderZ = document.getElementById('camera-z-slider') as HTMLInputElement;
                if (sliderZ) sliderZ.value = String(this.camera.z);
                this.render3DView();
            });
        },
        ...
        toggle3DView() {
            ...
            const section = document.getElementById('canvas-3d-section') as HTMLElement;
            const toggleBtn = document.getElementById('toggle-3d-btn') as HTMLButtonElement;
            const rotateBtn = document.getElementById('rotate-3d-btn') as HTMLButtonElement;
            const resetBtn = document.getElementById('reset-camera-btn') as HTMLButtonElement;
            const cameraControls = document.getElementById('3d-camera-controls') as HTMLElement;

            if (this.isActive) {
                if (section) section.style.display = 'block';
                if (toggleBtn) toggleBtn.textContent = this.util.t ? this.util.t('hide_3d') || 'Hide 3D View' : 'Hide 3D View';
                ...
                if (toggleBtn) toggleBtn.textContent = this.util.t ? this.util.t('launch_3d') || 'Launch 3D View' : 'Launch 3D View';
        ...
        toggleAutoRotate() {
            ...
            const btn = document.getElementById('rotate-3d-btn') as HTMLButtonElement;
            if (btn) {
                btn.textContent = this.autoRotate ?
                    ((this.util.t ? this.util.t('stop_rotate') : 'Stop Rotation') || 'Stop Rotation') :
                    ((this.util.t ? this.util.t('auto_rotate') : 'Auto Rotate') || 'Auto Rotate');
            }
        },
        ...
        resetCamera() {
            ...
            const sliderY = document.getElementById('camera-y-slider') as HTMLInputElement;
            const sliderZ = document.getElementById('camera-z-slider') as HTMLInputElement;
            const sliderFov = document.getElementById('fov-slider') as HTMLInputElement;

            if (sliderX) sliderX.value = '0';
            if (sliderY) sliderY.value = '0';
            if (sliderZ) sliderZ.value = '-500';
            if (sliderFov) sliderFov.value = '500';
        ...
        initialize3DData() {
            ...
            if (this.state.networkLayout && this.state.networkLayout.length > 0) {
                ...
                this.state.networkLayout.forEach((node: any, index: number) => {
                ...
                if (this.state.synapses) {
                    this.state.synapses.forEach((synapse: any) => {
         ...
         */
        getRegionVertices(region: string): number[] {
            const indices: number[] = [];
            if (!this.brainShell) return indices;
        ...
        start3DAnimation() {
            const animate = () => {
                ...
                if (this.autoRotate) {
                    if (this.camera.rotationY !== undefined) this.camera.rotationY += this.rotationSpeed;
                    const sliderY = document.getElementById('camera-y-slider') as HTMLInputElement;
                    if (sliderY) {
                        sliderY.value = String((window as any).GreenhouseModels3DMath.radToDeg(this.camera.rotationY || 0));
                    }
                }
        ...
        render3DView() {
            ...
            const projectedNeurons: Neuron3D[] = [];
            this.neurons3D.forEach(neuron => {
                const projected = (window as any).GreenhouseModels3DMath.project3DTo2D(
            ...
            projectedNeurons.sort((a, b) => (b.depth || 0) - (a.depth || 0));
         ...
         */
        drawBrainShell(ctx: CanvasRenderingContext2D) {
            ...
            facesWithDepth.sort((a, b) => (b ? b.depth : 0) - (a ? a.depth : 0));

            // Draw each face
            facesWithDepth.forEach((f) => {
                if (!f) return;
                const { face, depth, vertices } = f;
                ...
                if (cross > 0) {
                    ...
                    // Apply depth fog
                    let alpha = (window as any).GreenhouseModels3DMath.applyDepthFog(0.3, depth);
         ...
         */
        draw3DParticles(ctx: CanvasRenderingContext2D) {
         ...
         */
        draw3DGrid(ctx: CanvasRenderingContext2D) {
            ...
            for (let x = -gridExtent; x <= gridExtent; x += gridSize) {
                const start = (window as any).GreenhouseModels3DMath.project3DTo2D(
                    x, -100, -gridExtent,
                    this.camera, this.projection
                );
                const end = (window as any).GreenhouseModels3DMath.project3DTo2D(
                    x, -100, gridExtent,
                    this.camera, this.projection
                );
         ...
         */
        draw3DConnections(ctx: CanvasRenderingContext2D, projectedNeurons: Neuron3D[]) {
            this.connections3D.forEach(conn => {
                ...
                if (fromProj && toProj) {
                    const avgDepth = ((fromProj.depth || 0) + (toProj.depth || 0)) / 2;
                    const alpha = (window as any).GreenhouseModels3DMath.applyDepthFog(0.3, avgDepth);
                    ...
                    ctx.beginPath();
                    ctx.moveTo(fromProj.screenX || 0, fromProj.screenY || 0);
                    ctx.lineTo(toProj.screenX || 0, toProj.screenY || 0);
                    ctx.stroke();
                }
            });
        },
         ...
         * Draws 3D neurons
         */
        draw3DNeurons(ctx: CanvasRenderingContext2D, projectedNeurons: Neuron3D[]) {
            projectedNeurons.forEach(neuron => {
                const depth = neuron.depth !== undefined ? neuron.depth : 500;
                const scale = neuron.scale !== undefined ? neuron.scale : 1;
                const screenX = neuron.screenX !== undefined ? neuron.screenX : 0;
                const screenY = neuron.screenY !== undefined ? neuron.screenY : 0;

                const alpha = (window as any).GreenhouseModels3DMath.applyDepthFog(1, depth);
         ...
         */
        draw3DAxisIndicators(ctx: CanvasRenderingContext2D) {
            const origin = (window as any).GreenhouseModels3DMath.project3DTo2D(0, 0, 0, this.camera, this.projection);
            const xAxis = (window as any).GreenhouseModels3DMath.project3DTo2D(100, 0, 0, this.camera, this.projection);
            const yAxis = (window as any).GreenhouseModels3DMath.project3DTo2D(0, 100, 0, this.camera, this.projection);
            const zAxis = (window as any).GreenhouseModels3DMath.project3DTo2D(0, 0, 100, this.camera, this.projection);
         ...
         */
        setHoveredRegion(regionKey: string | null) {
    ...
    (window as any).GreenhouseModelsUI3D = GreenhouseModelsUI3D;
})();
