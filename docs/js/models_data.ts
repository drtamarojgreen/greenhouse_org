...
interface Lexicon {
    ...
    neurotransmitter_affinity: Record<string, string[]>;
}

interface SynapseData {
    elements: any[];
    ionChannels: any[];
    cytoplasmDensity: number;
    localRNA: boolean;
}
...
interface ProcessedNode {
    ...
    label: string;
    domain_tags: string[];
    strength_prior: number;
    neuro_affinity: string[];
    notes_ref: string[];
}

interface ProcessedSynapse {
    id: string;
    pre: string;
    post: string;
    weight: number;
    plasticity_rate: number;
...
interface ProcessedSimulation {
    session_id: string;
    consent: boolean;
    created_at: string;
    nodes: ProcessedNode[];
    synapses: ProcessedSynapse[];
...
const GreenhouseModelsData = {
    state: {
        ...
        lexicon: null as Lexicon | null,
        processedSimulation: null as ProcessedSimulation | null,
        synapseData: null as SynapseData | null,
        brainData: null as BrainData | null,
        environmentData: null as any | null,
        synapticWeight: 0.5,
        neurotransmitters: 0,
        ionsCrossed: 0,
        learningMetric: 0,
    ...
    async loadData(): Promise<void> {
        try {
            const dataElement = await new Promise<HTMLElement | null>((resolve) => {
                let elapsedTime = 0;
                const poll = setInterval(() => {
                    const element = document.querySelector('section.wixui-section:nth-child(2) > div:nth-child(2) > div:nth-child(1) > div:nth-child(1) > span:nth-child(1)') as HTMLElement | null;
            ...
            if (dataElement) {
                ...
                const veloData = JSON.parse(dataElement.dataset.customHolder || '{}') as SimulationData;
                this.state.simulationData = veloData;
                this.state.lexicon = veloData.lexicon || { domain_tags: {}, neurotransmitter_affinity: {} };
                this.state.synapseData = veloData.synapse || null;
                this.state.brainData = veloData.brain || null;
                this.state.environmentData = veloData.environment || null;
            } else {
            ...
            this.state.lexicon = { domain_tags: {}, neurotransmitter_affinity: {} };
            this.state.synapseData = null;
            this.state.brainData = { elements: [] };
            this.state.environmentData = {};
        }
    },

    transformNotesToSimulationInput(rawNotes: Note[] | null, lexicon: Lexicon | null): ProcessedSimulation {
        console.log("Starting transformation pipeline...");

        const simulation: ProcessedSimulation = {
        ...
        const tempNodeMap = new Map<string, ProcessedNode>();

        // Helper functions
        const sanitize = (text: string) => text.replace(/[^a-zA-Z0-9\s-]/g, '');
        const createLabel = (text: string) => sanitize(text).substring(0, 20).replace(/\s+/g, '_').toUpperCase();
        const extractTags = (text: string, tagMap: Record<string, string[]> | undefined) => {
            const foundTags: string[] = [];
            if (!tagMap) return foundTags;
        ...
        if (rawNotes) {
            rawNotes.forEach(note => {
                ...
                if (!simulation.meta.source_counts[sourceType]) {
                    simulation.meta.source_counts[sourceType] = 0;
                }
                ...
                if (sourceType === "user") {
                    const nodeObj = tempNodeMap.get(nodeLabel);
                    const nodeId = nodeObj ? nodeObj.id : '';
        ...
        if (simulation.nodes.length >= 2) {
            const synapse: ProcessedSynapse = {
                id: "syn-1",
    ...
    update(networkState: NetworkState): NetworkState | undefined {
        if (!this.state.processedSimulation) return;
...
(window as any).GreenhouseModelsData = GreenhouseModelsData;
