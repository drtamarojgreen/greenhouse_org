...
(function () {
    ...
    const GreenhouseCognitionResearch = {
        ...
        render(ctx) {
            ...
            if (!activeEnhancement || activeEnhancement.category !== 'Research') return;
            ...
            else if (activeEnhancement.id === 153) this.renderLiteratureSearch(ctx, w, h);
            else if (activeEnhancement.id === 154) this.renderDataExport(ctx, w, h);
            else if (activeEnhancement.id === 156) this.renderDesignAssistant(ctx, w, h);
            else if (activeEnhancement.id === 163) this.renderStatisticsTool(ctx, w, h);
            else {
                ctx.fillStyle = '#ccc';
        ...
        renderHypothesisSandbox(ctx, w, h) {
            ctx.fillStyle = '#111';
            ctx.fillRect(40, 110, w - 80, 120);
            ctx.strokeStyle = '#4fd1c5';
            ctx.strokeRect(40, 110, w - 80, 120);
        ...
        renderDataGenerator(ctx, w, h) {
            ctx.fillStyle = '#222';
            ctx.fillRect(40, 110, w - 80, 130);
            ctx.strokeStyle = '#fff';
            ctx.strokeRect(40, 110, w - 80, 130);

            ctx.fillStyle = '#4da6ff';
            ctx.font = 'bold 10px Arial';
            ctx.fillText('SUBJECT_ID | REGION | VALUE | TIMESTAMP', 50, 125);
            ctx.beginPath();
            ctx.moveTo(40, 130); ctx.lineTo(w - 40, 130); ctx.stroke();
        ...
        renderDataExport(ctx, w, h) {
            ...
            ctx.fillText('{', 60, 125);
            ctx.fillText('  "experiment": "Cognition_v2",', 60, 135);
            ctx.fillText('  "data": [', 60, 145);
            ctx.fillText('    {"id": 1, "score": 0.92},', 60, 155);
            ctx.fillText('    {"id": 2, "score": 0.85}', 60, 165);
            ctx.fillText('  ]', 60, 175);
            ctx.fillText('}', 60, 185);
        ...
        renderDesignAssistant(ctx, w, h) {
            ...
            ctx.fillStyle = '#fff';
            ctx.fillText('> Dorsolateral PFC', 60, 207);
        },

        renderStatisticsTool(ctx, w, h) {
            ctx.strokeStyle = '#fff';
            ctx.beginPath();
            ctx.moveTo(50, 210); ctx.lineTo(150, 210); ctx.moveTo(50, 210); ctx.lineTo(50, 130);
            ctx.stroke();
            // Normal curve
            ctx.beginPath();
            ctx.strokeStyle = '#4fd1c5';
            for(let i=0; i<100; i++) {
                const x = 50 + i;
                const y = 210 - 60 * Math.exp(-Math.pow(i-50, 2)/400);
        ...
        drawGenericPortal(ctx, w, h) {
            ...
            ctx.beginPath();
            ctx.arc(w/2, h/2 + 30, 40 + Math.sin(time)*10, 0, Math.PI*2);
            ctx.stroke();
            ctx.beginPath();
            ctx.arc(w/2, h/2 + 30, 20 + Math.cos(time)*5, 0, Math.PI*2);
            ctx.stroke();
        }
    };
