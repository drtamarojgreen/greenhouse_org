    ...
    const GreenhouseCognitionMedications = {
        ...
        setupMappings() {
            this.mappings = {
                ...
                82: (ctx, w, h) => utils().drawSynapse(ctx, w * 0.5, h * 0.5, '#E0E0E0', t('cog_label_d2_blockade')),
                83: (ctx, w, h) => utils().drawSynapse(ctx, w * 0.5, h * 0.5, '#FFFFFF', t('cog_label_increased_ne_da')),
                84: (ctx, w, h) => utils().drawSynapse(ctx, w * 0.5, h * 0.5, '#E0E0E0', t('cog_label_gaba_potentiation')),
                85: (ctx, w, h) => utils().drawPulse(ctx, w * 0.5, h * 0.5, '#ffffff', t('cog_label_signal_stabilization')),
                86: (ctx, w, h) => utils().drawSynapse(ctx, w * 0.5, h * 0.5, '#D0D0D0', t('cog_label_dual_action')),
                87: (ctx, w, h) => utils().drawSynapticGrowth(ctx, w * 0.5, h * 0.5, '#FFFFFF', t('cog_label_synaptogenesis_burst')),
                88: (ctx, w, h) => utils().drawSynapse(ctx, w * 0.5, h * 0.5, '#FFFFFF', t('cog_label_mao_inhibition')),
                89: (ctx, w, h) => utils().drawSynapse(ctx, w * 0.5, h * 0.5, '#A0AEC0', t('cog_label_ach_inhibition')),
                90: (ctx, w, h) => {
                ...
                91: (ctx, w, h) => {
                    ...
                    for (let i = 0; i < 8; i++) {
                        ...
                        ctx.fillStyle = (i % 3 === 0) ? '#E0E0E0' : '#4da6ff';
                ...
                96: (ctx, w, h) => {
                    const x = w * 0.5 - 100, y = h * 0.5 - 60, color = '#A0AEC0';
                    const sw = 200, sh = 120;
                ...
                97: (ctx, w, h) => {
                    ...
                    ctx.fillStyle = '#E0E0E0'; ctx.fillText(t('cog_label_complex_interaction'), x - 70, y + 60);
                },
                98: (ctx, w, h) => {
                    const x = w * 0.5, y = h * 0.5, color = '#D0D0D0';
                ...
                99: (ctx, w, h) => {
                    const x = w * 0.5, y = h * 0.5;
                    ctx.fillStyle = '#D0D0D0'; ctx.beginPath(); ctx.arc(x, y, 15, 0, Math.PI * 2); ctx.fill();
                    ctx.fillText(t('cog_label_metabolic_impact'), x - 70, y + 40);
                },
                100: (ctx, w, h) => {
                    const x = w * 0.5, y = h * 0.5;
                    ctx.fillStyle = '#A0AEC0'; ctx.beginPath(); ctx.arc(x, y, 10, 0, Math.PI * 2); ctx.fill();
                    ctx.strokeStyle = '#A0AEC0'; ctx.beginPath(); ctx.arc(x, y, 20, 0, Math.PI * 2); ctx.stroke();
