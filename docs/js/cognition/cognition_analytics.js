...
(function () {
    ...
    const GreenhouseCognitionAnalytics = {
        ...
        setupMappings() {
            this.mappings = {
                ...
                2: (ctx, w, h) => {
                    ...
                    ctx.fillStyle = '#fff';
                    ctx.beginPath();
                    ctx.arc(100 + (w - 200) * progress, h / 2, 6, 0, Math.PI * 2);
                    ctx.fill();
                },
                3: (ctx, w, h) => {
                    ...
                    ctx.fillText(t('cog_label_system1'), w * 0.2, 120);
                    ctx.fillStyle = '#4da6ff';
                    ctx.fillText(t('cog_label_system2'), w * 0.7, 120);
                },
                4: (ctx, w, h) => {
                    ctx.strokeStyle = '#A0AEC0';
                    ctx.strokeRect(w - 150, 50, 120, 150);
                    ctx.fillStyle = '#fff';
                    ctx.fillText(t('cog_label_hud'), w - 145, 70);
                },
                5: (ctx, w, h) => {
                    ...
                    ctx.fill();
                    ctx.fillStyle = '#E0E0E0';
                    ctx.fillText(t('cog_label_deficit'), w / 2 - 50, h / 2 + 80);
                },
                6: (ctx, w, h) => {
        ...
        drawWorkingMemory(ctx, w, h, time) {
            ...
            ctx.fillStyle = '#fff';
            ctx.fillText(t('cog_label_working_memory_buffer'), w / 2 - 80, h / 2 - 60);
            for (let i = 0; i < 5; i++) {
                const x = w / 2 - 80 + i * 35;
        ...
        drawLoadMeter(ctx, w, h, time) {
            ...
            ctx.fillStyle = '#fff';
            ctx.fillText(t('cog_label_cognitive_load') || 'COGNITIVE LOAD:', w / 2 - 100, h / 2 - 20);
            ctx.strokeStyle = '#fff';
        ...
        drawPlasticity(ctx, w, h, time) {
            ...
            ctx.stroke();
            ctx.fillStyle = '#fff';
            ctx.fillText(t('cog_label_synaptic_strengthening') || 'Synaptic Strengthening (LTP)', w / 2 - 70, h / 2 + 60);
            ctx.restore();
        },
        drawLTMFormation(ctx, w, h, time) {
            ctx.fillStyle = '#fff';
            ctx.fillText(t('cog_label_ltm_consolidating') || 'Consolidating...', w / 2 - 100, h / 2 - 40);
            ...
            ctx.fillStyle = '#f6e05e';
            ctx.beginPath();
            ctx.arc((w / 2 - 50) + (100 * progress), h / 2, 5, 0, Math.PI * 2);
            ctx.fill();
        },
        drawAttentionNetwork(ctx, w, h, time) {
            ...
            ctx.fill();
            ctx.fillStyle = '#fff';
            ctx.fillText(t('cog_label_attention_focus'), focusX - 45, focusY - 25);
        },
        drawLanguagePath(ctx, w, h, time) {
            ...
            ctx.stroke();
            ctx.setLineDash([]);
            ctx.fillText(t('cog_label_arcuate'), w * 0.45, h * 0.3);
        },
        drawOscillation(ctx, w, h, time) {
            ...
            ctx.stroke();
            ctx.fillStyle = '#4da6ff';
            ctx.fillText(t('cog_label_gamma'), 50, h / 2 - 40);
        },
        ...
        drawBiasSimulator(ctx, w, h, time) {
            ctx.fillStyle = '#fff';
            ctx.fillText(t('cog_label_bias_confirmation'), 30, 100);
            ctx.strokeStyle = '#E0E0E0';
