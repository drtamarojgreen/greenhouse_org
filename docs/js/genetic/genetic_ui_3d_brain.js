...
(function () {
    ...
    const GreenhouseGeneticBrain = {
        ...
        drawBrainShell(ctx, brainShell, camera, projection, width, height, activeGene = null) {
            ...
            const material = (window.GreenhouseGeneticConfig && window.GreenhouseGeneticConfig.get) ?
                window.GreenhouseGeneticConfig.get('materials.brain') :
        ...
        drawRegionLabels(ctx, brainShell, camera, projection) {
            ...
            Object.entries(brainShell.regions).forEach(([key, data]) => {
                ...
                if (p.scale > 0.4 && p.x > 0 && p.x < projection.width) {
                    const label = brainShell.regionalPlanes?.find(rp => rp.label.toLowerCase().includes(key.substring(0, 3)))?.label ||
