...
(function () {
    ...
    const GreenhouseGeneticBrain = {
        ...
        drawTargetView(ctx, x, y, w, h, activeGene, activeGeneIndex, brainShell, drawPiPFrameCallback, cameraState) {
            ...
            if (!brainShell || !cameraState || !cameraState.camera)
                return;
        ...
        drawOrientationWidget(ctx, camera, width, height) {
            ...
            axes.forEach(axis => {
                const p = GreenhouseModels3DMath.project3DTo2D(axis.x * 30, axis.y * 30, axis.z * 30, { x: 0, y: 0, z: -100, rotationX: camera.rotationX, rotationY: camera.rotationY, rotationZ: camera.rotationZ, fov: 200 }, { width: 0, height: 0, near: 1, far: 1000 });
                ctx.beginPath();
        ...
        drawNeuron(ctx, p, neuronMeshes, camera, projection) {
            ...
            facesWithDepth.forEach(({ faceIndices }) => {
                ...
                if (!p0 || !p1 || !p2 || p0.scale <= 0 || p1.scale <= 0 || p2.scale <= 0)
                ...
                if (crossProduct >= 0)
                    return; // Only draw front-facing triangles
                ctx.beginPath();
