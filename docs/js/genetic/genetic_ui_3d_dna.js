        ...
        drawMacroView(ctx, w, h, camera, projection, neurons3D, activeGeneIndex, brainShell, drawNeuronCallback, drawBrainShellCallback, drawPiPFrameCallback, cameraState) {
            ...
            neurons3D.forEach((n, i) => {
                ...
                if (p.scale > 0) {
                    ...
                    projectedGenes.push(Object.assign(Object.assign(Object.assign({}, n), p), { isFocused }));
                }
            });
            ...
            for (let i = 0; i < helixNodes.length; i += 2) {
                ...
                if (p1.scale > 0 && p2.scale > 0) {
                    ...
                    const drawStructuralRung = (x1, y1, x2, y2, depth) => {
                        ...
                        const gradient = ctx.createLinearGradient(x1 - rx * rungThickness / 2, y1 - ry * rungThickness / 2, x1 + rx * rungThickness / 2, y1 + ry * rungThickness / 2);
                        ...
                        if (type === 0) { // Adenine (Box cap)
                            ...
                            ctx.fillStyle = litColor;
                            ctx.beginPath();
                            ctx.moveTo(x2, y2 - thickness * 0.5);
                            ctx.lineTo(x2 + thickness * 0.4, y2 + thickness * 0.3);
                            ctx.lineTo(x2 - thickness * 0.4, y2 + thickness * 0.3);
                            ctx.fill();
                        }
                            ...
                            ctx.beginPath();
                            ctx.moveTo(x2, y2 - thickness * 0.5);
                            ctx.lineTo(x2 + thickness * 0.4, y2);
                            ctx.lineTo(x2, y2 + thickness * 0.5);
                            ctx.lineTo(x2 - thickness * 0.4, y2);
                            ctx.closePath();
                            ctx.fill();
                        }
