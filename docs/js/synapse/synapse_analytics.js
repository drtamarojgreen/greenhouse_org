...
(function () {
    ...
    G.Analytics = {
        ...
        update(particleCount, ionCount, activeReceptors) {
            ...
            if (this.state.calcium > 2.0)
                this.state.calcium = 2.0;
            ...
            if (scoreElem)
                scoreElem.innerText = this.state.health.toFixed(1);
            const caElem = document.getElementById('calcium-level');
            if (caElem)
                caElem.innerText = `${this.state.calcium.toFixed(2)} μM`;
            const currentElem = document.getElementById('membrane-current');
            if (currentElem)
                currentElem.innerText = `${this.state.membraneCurrent.toFixed(1)} pA`;
            const sensElem = document.getElementById('sensitivity-index');
            if (sensElem)
                sensElem.innerText = this.state.sensitivity.toFixed(2);
        ...
        updateLiterature() {
            ...
            if (!panel || !((_a = G.config.visuals) === null || _a === void 0 ? void 0 : _a.showLiterature)) {
                if (panel)
