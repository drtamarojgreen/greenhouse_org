...
(function () {
    ...
    Object.assign(G, {
        ...
        getSurfaceY(h) {
            var _a;
            const cleft = ((_a = this.config.kinetics) === null || _a === void 0 ? void 0 : _a.cleftWidth) || 1.0;
        ...
        handleMouseDown(e) {
            ...
            if ((_a = G.config.visuals) === null || _a === void 0 ? void 0 : _a.annotationMode) {
                const note = prompt('Enter research annotation:');
                if (note)
                    this.annotations.push({ x: this.mouse.x, y: this.mouse.y, text: note });
                return;
            }
            const w = this.canvas.width / (window.devicePixelRatio || 1);
            const h = this.canvas.height / (window.devicePixelRatio || 1);
        ...
        render() {
            ...
            const w = this.canvas.width / (window.devicePixelRatio || 1);
            const h = this.canvas.height / (window.devicePixelRatio || 1);
            const nightDim = ((_a = G.config.visuals) === null || _a === void 0 ? void 0 : _a.isNight) ? 0.3 : 1.0;
            ctx.fillStyle = G.config.highContrast ? '#000' : (((_b = G.config.visuals) === null || _b === void 0 ? void 0 : _b.isNight) ? '#000500' : '#010501');
            ctx.fillRect(0, 0, w, h);
            if (G.Molecular) {
                ctx.globalAlpha = nightDim;
                G.Molecular.drawECM(ctx, w, h);
                G.Molecular.drawAstrocyte(ctx, w, h);
                G.Molecular.drawMitochondria(ctx, w * 0.4, h * 0.15, ((_d = (_c = G.Analytics) === null || _c === void 0 ? void 0 : _c.state) === null || _d === void 0 ? void 0 : _d.atp) || 100);
                ctx.globalAlpha = 1.0;
            }
            if ((_e = G.config.visuals) === null || _e === void 0 ? void 0 : _e.fluorescenceActive) {
                const caLevel = ((_g = (_f = G.Analytics) === null || _f === void 0 ? void 0 : _f.state) === null || _g === void 0 ? void 0 : _g.calcium) || 0.1;
                ctx.save();
            ...
            if (G.Visuals3D) {
                ...
                if ((_h = G.config.visuals) === null || _h === void 0 ? void 0 : _h.showElectrostatic) {
                    G.Visuals3D.drawElectrostaticPotential(ctx, w, h, this.frame);
                }
                if ((_j = G.config.pharmacology) === null || _j === void 0 ? void 0 : _j.bbbActive) {
            ...
            if (G.Molecular) {
                const chol = ((_k = G.config.kinetics) === null || _k === void 0 ? void 0 : _k.cholesterol) || 1.0;
                ...
                G.Molecular.drawScaffolding(ctx, w, h, G.Particles.plasticityFactor, (_l = G.config.visuals) === null || _l === void 0 ? void 0 : _l.showIsoforms);
                ...
                if ((_m = G.config.visuals) === null || _m === void 0 ? void 0 : _m.patchClampActive) {
            ...
            if (G.Particles) {
                ...
                const azDensity = ((_o = G.config.kinetics) === null || _o === void 0 ? void 0 : _o.activeZoneDensity) || 0.04;
                const caLevel = ((_q = (_p = G.Analytics) === null || _p === void 0 ? void 0 : _p.state) === null || _q === void 0 ? void 0 : _q.calcium) || 0.1;
                const releaseProb = (((_s = (_r = G.Chemistry.scenarios[G.config.activeScenario]) === null || _r === void 0 ? void 0 : _r.modifiers) === null || _s === void 0 ? void 0 : _s.releaseProb) || 0.5) * (caLevel * 5) * retrogradeInhibition;
                if (this.frame % 15 === 0 && Math.random() < releaseProb) {
                    ...
                    if (p)
                        p.x = w * (0.5 - azDensity / 2 + Math.random() * azDensity);
                }
                ...
                if (G.Analytics)
                    G.Analytics.update(G.Particles.particles.length, G.Particles.ions.length, activeReceptors);
                if (G.Visuals3D) {
        ...
        handleReceptorInteractions(w, h) {
            var _a, _b;
            if (!G.Particles || !G.Chemistry)
                return;
            const particles = G.Particles.particles;
            const chem = G.Chemistry;
            const surfaceY = this.getSurfaceY(h);
            const pharm = G.config.pharmacology || {};
            const pH = ((_a = G.config.kinetics) === null || _a === void 0 ? void 0 : _a.pH) || 7.4;
            const pH_modifier = Math.max(0.1, 1.0 - Math.abs(pH - 7.4) * 2);
            const circadian_modifier = ((_b = G.config.visuals) === null || _b === void 0 ? void 0 : _b.isNight) ? 0.7 : 1.0;
                ...
                particles.forEach(p => {
                    ...
                    if (dist < 20 && p.life > 0.1) {
                        ...
                        if (!canBind && pharm.offTargetActive) {
                            const affinity = ((_a = receptorType.offTargetAffinities) === null || _a === void 0 ? void 0 : _a[p.chemistry.id]) || 0;
                            if (Math.random() < affinity)
                                canBind = true;
                        }
                        if (canBind) {
                            const hill = ((_b = p.chemistry.kinetics) === null || _b === void 0 ? void 0 : _b.hill) || 1.0;
                            if (Math.random() > (pH_modifier * circadian_modifier * hill))
                                return;
                            p.life = 0;
                            if (pharm.antagonistActive && receptor.type === 'ionotropic_receptor')
                                return;
                            ...
                            if ((receptor.type === 'ionotropic_receptor' || receptor.type === 'ampar') && p.chemistry.ionEffect !== 'none') {
                                if (pharm.ttxActive && p.chemistry.ionEffect === 'sodium')
                                    return;
                                let ionsToCreate = (receptor.type === 'ampar') ? 2 : 1;
                                if (pharm.benzodiazepineActive && p.chemistry.id === 'gaba')
                                    ionsToCreate = 3;
                                ...
                                receptor.state = 'open';
                                setTimeout(() => { if (receptor.state === 'open')
                                    receptor.state = 'closed'; }, 200);
                                if (G.Particles.ions.length > 30 && Math.random() > 0.9) {
                                    if (G.Molecular)
                                        G.Molecular.triggerRetrograde(rx, ry);
                                }
                            }
                            else if (receptor.type === 'nmdar') {
                                // NMDA specific: requires depolarization > -30 (Mg2+ block release)
                                const depol = ((_d = (_c = G.Analytics) === null || _c === void 0 ? void 0 : _c.state) === null || _d === void 0 ? void 0 : _d.depolarization) || -70;
                                if (depol > -30) {
                                    G.Particles.createIon(rx, ry + 10, 'calcium');
                                    receptor.state = 'open';
                                    setTimeout(() => { if (receptor.state === 'open')
                                        receptor.state = 'closed'; }, 300);
                                }
                                else {
                                    receptor.state = 'blocked';
                                    setTimeout(() => { if (receptor.state === 'blocked')
                                        receptor.state = 'closed'; }, 200);
                                }
                            }
                                ...
                                setTimeout(() => { if (receptor.state === 'active')
                                    receptor.state = 'idle'; }, 500);
                            }
                        }
                    }
                });
        ...
        checkHover(w, h) {
            ...
            else if (my > surfaceY - 10 && Math.abs(mx - w * 0.5) < w * 0.28)
            ...
            G.config.elements.vesicles.forEach(v => {
                ...
                if (Math.pow((mx - vx), 2) + Math.pow((my - vy), 2) < Math.pow(v.r, 2) * 2)
                    this.hoveredId = 'vesicle';
            });
            // Autoreceptors
            G.config.elements.autoreceptors.forEach(ar => {
                const ax = w * ar.x, ay = h * ar.y;
                if (Math.pow((mx - ax), 2) + Math.pow((my - ay), 2) < 144)
                    this.hoveredId = 'autoreceptor';
            });
            // Transporters
            G.config.elements.transporters.forEach(tr => {
                const tx = w * tr.x, ty = h * tr.y;
                if (Math.abs(mx - tx) < 12 && Math.abs(my - ty) < 15)
                    this.hoveredId = tr.type;
            });
            // Receptors
            G.config.elements.receptors.forEach(r => {
                const rx = w * r.x, ry = surfaceY - 5;
                if (Math.abs(mx - rx) < 15 && Math.abs(my - ry) < 20)
                    this.hoveredId = r.type;
            });
            // Molecular Structures
            if (Math.pow((mx - w * 0.4), 2) + Math.pow((my - h * 0.15), 2) < 1600)
                this.hoveredId = 'mitochondria';
            if (Math.pow((mx - w * 0.8), 2) + Math.pow((my - h * 0.5), 2) < 2500)
                this.hoveredId = 'astrocyte';
            if (my > surfaceY && my < surfaceY + 40 && Math.abs(mx - w * 0.5) < w * 0.2)
                this.hoveredId = 'psd95';
            if (Math.abs(my - h * 0.44) < 10 && Math.abs(mx - w * 0.5) < w * 0.2)
                this.hoveredId = 'lipidBilayer';
            if (Math.abs(my - surfaceY) < 10 && Math.abs(mx - w * 0.5) < w * 0.3)
                this.hoveredId = 'lipidBilayer';
            if (Math.abs(mx - w * 0.2) < 30 && Math.abs(my - h * 0.5) < 40)
                this.hoveredId = 'gradient';
            if (Math.abs(mx - w * 0.5) < 25 && Math.abs(my - h * 0.4) < 25)
                this.hoveredId = 'snare';
            // Particles & Ions
            if (G.Particles) {
                G.Particles.ions.forEach(ion => {
                    if (Math.pow((mx - ion.x), 2) + Math.pow((my - ion.y), 2) < 144)
                        this.hoveredId = ion.type || 'sodium';
                });
            }
            if (G.Molecular && G.Molecular.retrogradeSignals) {
                G.Molecular.retrogradeSignals.forEach(s => {
                    if (Math.pow((mx - s.x), 2) + Math.pow((my - s.y), 2) < 144)
                        this.hoveredId = 'retrograde';
                });
            }
        },
        drawStructure(ctx, w, h) {
            ...
            G.config.elements.transporters.forEach(tr => {
                var _a;
                let color = '#E0E0E0';
                if (tr.type === 'SERT' && ((_a = G.config.pharmacology) === null || _a === void 0 ? void 0 : _a.ssriActive))
                    color = '#ff4444';
                if (tr.type.startsWith('VMAT'))
                    color = '#A0AEC0'; // Purple for VMAT
                if (tr.type === 'NET')
                    color = '#FF5722'; // Deep Orange for NET
                ctx.fillStyle = color;
            ...
            G.config.elements.receptors.forEach(r => {
                ...
                if (r.type === 'ionotropic_receptor') {
                    ...
                    ctx.fillRect(rx - 8, ry, 3, 12);
                    ctx.fillRect(rx - 4, ry, 3, 12);
                    ctx.fillRect(rx + 1, ry, 3, 12);
                    ctx.fillRect(rx + 5, ry, 3, 12);
                }
