...
(function () {
    ...
    const GreenhouseInflammationMacro = {
        ...
        drawCompartmentFlowNetwork(ctx, state, camera, projection, ui3d) {
            ...
            if (!graph || !graph.compartments || !graph.reactions)
                return;
        ...
        drawEliteBrain(ctx, shell, camera, projection, tone, ui3d) {
            ...
            faces.forEach(f => {
                ...
                if (hemi === 'left' && !ui3d.app.ui.showLeftHemisphere)
                    return;
                if (hemi === 'right' && !ui3d.app.ui.showRightHemisphere)
                    return;
                if (deepKeys.includes(regionKey) && !ui3d.app.ui.showDeepStructures)
                    return;
