    ...
    const GreenhouseInflammationUI3D = {
        ...
        loadPathways() {
            return __awaiter(this, void 0, void 0, function* () {
                try {
                    const data = yield window.GreenhouseModelsUtil.PathwayService.loadMetadata(this.app.baseUrl);
                    this.availablePathways = data.pathways;
                    // Pre-load default pathway for inflammation
                    yield this.fetchPathway('tryptophan');
                }
                catch (e) {
                    console.error("Inflammation UI: Failed to load pathway metadata", e);
                    if (window.GreenhouseInflammationPathway)
                        window.GreenhouseInflammationPathway.error = "Metadata Load Failed";
                }
            });
        },
        fetchPathway(id) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                if (!meta) {
                    if (window.GreenhouseInflammationPathway)
                        window.GreenhouseInflammationPathway.error = "Pathway Not Found: " + id;
                    return;
                }
                if (!meta.source) {
                ...
                try {
                    if (meta.source.endsWith('.json')) {
                        data = yield window.GreenhouseModelsUtil.PathwayService.loadJSONPathway(meta.source, this.app.baseUrl);
                        if (data) {
                        ...
                        data = yield window.GreenhouseModelsUtil.PathwayService.loadPathway(meta.source, this.app.baseUrl);
                    }
                }
                catch (e) {
                ...
                if (data) {
                    const nodesWithPos = data.nodes.map((n, i) => {
                        ...
                        return Object.assign(Object.assign({}, n), { x: base.x + (Math.sin(i) * 20), y: base.y + (Math.cos(i) * 20), z: base.z + (Math.sin(i * 0.5) * 10) });
                    });
        ...
        initMicroData() {
            ...
            for (let i = 0; i < this.neurons.length - 1; i++) {
                if (Math.random() > 0.3)
                    this.axons.push({ from: i, to: (i + 1) % this.neurons.length });
            }
            this.leukocytes = [];
            for (let i = 0; i < 12; i++) {
        ...
        render(ctx, state, camera, projection) {
            ...
            if (viewMode === 'pathway') {
                ...
                if (state.factors.showBridgeOverlay)
                    this.drawMolecularToMicroBridge(ctx, state, projection);
            }
        },
        ...
        checkHover(mx, my, camera, projection) {
            ...
            if (viewMode === 'micro') {
                for (const g of this.glia) {
                    ...
                    if (dist < 25 * p.scale)
                        return { id: 'glia', label: g.type === 'astrocyte' ? 'Astrocyte' : 'Microglia', description: g.type === 'astrocyte' ? 'Glia that supports neurons and maintains the blood-brain barrier via endfeet.' : 'Resident immune cell; M1 (reactive) or M2 (resolving) phenotypes.', type: '3d', data: g };
                }
                for (const l of this.leukocytes) {
                    const p = Math3D.project3DTo2D(l.x, l.y, l.z, camera, projection);
                    const dist = Math.sqrt(Math.pow((p.x - mx), 2) + Math.pow((p.y - my), 2));
                    if (dist < 15 * p.scale)
                        return { id: 'leukocyte', label: 'Leukocyte', description: 'White blood cell involved in the inflammatory response.', type: '3d', data: l };
                }
                if (my > projection.height * 0.7)
                    return { id: 'bbb', label: 'Blood-Brain Barrier', description: 'Endothelial lining and basement membrane; regulates leukocyte infiltration.', type: '3d' };
            }
            else if (viewMode === 'molecular') {
                if (my > projection.height * 0.3 && my < projection.height * 0.5)
                    return { id: 'membrane', label: 'Cell Membrane', description: 'Lipid bilayer barrier separating the intracellular and extracellular milieu.', type: '3d' };
                if (this.molecularCells) {
                    for (const c of this.molecularCells) {
                        ...
                        if (dist < 40 * p.scale) {
                            ...
                            if (c.type === 'mast_cell') {
                                label = 'Mast Cell';
                                desc = 'Immune cell containing granules rich in histamine and cytokines.';
                            }
                            else if (c.type === 'astrocyte') {
                                label = 'Astrocyte';
                                desc = 'Star-shaped glial cell that provides structural and metabolic support.';
                            }
                            else if (c.type === 'microglia') {
                                label = 'Microglia';
                                desc = 'Resident immune cell of the CNS; monitors and responds to injury.';
                            }
                ...
                for (const m of this.molecules) {
                    ...
                    if (dist < 12 * p.scale) {
                        ...
                        if (m.type === 'pro-cytokine') {
                            label = 'Pro-Cytokine (TNF-α)';
                            desc = 'Aggressive signaling molecule that promotes inflammatory cascades.';
                        }
                        else if (m.type === 'anti-cytokine') {
                            label = 'Anti-Cytokine (IL-10)';
                            desc = 'Protective molecule that resolves inflammation and prevents tissue damage.';
                        }
                        else if (m.type === 'neurotransmitter') {
                            label = 'Neurotransmitter';
                            desc = 'Chemical messenger released across synapses to signal between neurons.';
                        }
                ...
                for (const key in this.brainShell.regions) {
                    ...
                    if (region.centroid) {
                        ...
                        if (dist < 50 * p.scale && p.depth < 0.7)
                            return { id: 'brain_region', label: region.name, description: window.GreenhouseModelsUtil ? (window.GreenhouseModelsUtil.getRegionDescription(key) || 'Functional brain region.') : 'Brain Region', type: '3d' };
                    }
                }
            }
            return null;
