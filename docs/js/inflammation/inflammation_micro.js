...
(function () {
    ...
    const GreenhouseInflammationMicro = {
        render(ctx, state, camera, projection, ui3d) {
            ...
            batch.forEach(item => {
                if (item.type === 'endothelium')
                    this.drawEndothelium(ctx, item.data, item.proj, bbb);
                else if (item.type === 'neuron')
                    this.drawNeuron(ctx, item.data, camera, projection, tone);
                else if (item.type === 'glia')
                    this.drawGlia(ctx, item.data, item.proj, activation, tone, ui3d);
                else if (item.type === 'leukocyte')
                    this.drawLeukocyte(ctx, item.data, item.proj, bbb);
            });
            ...
            if (state.factors.showVolumeBounds)
                this.drawVolumeBoundingBox(ctx, camera, projection);
        },
        ...
        drawVolumeBoundingBox(ctx, camera, projection) {
            ...
            [0, 1, 2, 3].forEach(i => {
                ...
                ctx.lineTo(p[(i + 1) % 4].x, p[(i + 1) % 4].y);
                ctx.moveTo(p[i + 4].x, p[i + 4].y);
                ctx.lineTo(p[(i + 1) % 4 + 4].x, p[(i + 1) % 4 + 4].y);
                ctx.moveTo(p[i].x, p[i].y);
        ...
        drawLeukocyte(ctx, l, p, bbb) {
            ...
            if (l.state === 'infiltrating' && l.history) {
                ...
                l.history.forEach((h, i) => {
                    if (i === 0)
                        ctx.moveTo(h.proj.x, h.proj.y);
                    else
                        ctx.lineTo(h.proj.x, h.proj.y);
                });
                ctx.stroke();
                ctx.restore();
            }
