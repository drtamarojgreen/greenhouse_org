    ...
    const GreenhouseInflammationApp = {
        ...
        init(selector, baseUrl = '') {
            ...
            try {
                ...
                console.warn("GreenhouseInflammationApp: localStorage unavailable");
            }
            this.ui.categories = [
            ...
            this.ui.categories.forEach(cat => {
                ...
                if (saved)
                    cat.isOpen = saved.isOpen;
            });
            this.setupUI();
            this.computeUILayout();
            this.ui.baselineMetrics = Object.assign({}, this.engine.state.metrics);
        ...
        handleMouseDown(e) {
            ...
            if (openCat) {
                ...
                if (mx >= sx && mx <= sx + panelW - 20 && my >= sy && my <= sy + 20) {
                    ...
                    if (query !== null)
                        this.ui.searchQuery = query.toLowerCase();
                    return;
                }
            }
                ...
                for (let i = 0; i < Math.min(totalMetrics, this.ui.metricsPerPage); i++) {
                    ...
                    if (mx >= bx && mx <= bx + 100 && my >= by && my <= by + 60) {
                        ...
                        if (Analysis.selectedMetrics.includes(mid)) {
                            ...
                            if (Analysis.selectedMetrics.length >= 3)
                                Analysis.selectedMetrics.shift();
            ...
            for (const b of this.ui.buttons) {
                if (mx >= b.x && mx <= b.x + b.w && my >= b.y && my <= b.y + b.h) {
                    if (b.type === 'toggle') {
                        if (b.id === 'toggle_left')
                            this.ui.showLeftHemisphere = !this.ui.showLeftHemisphere;
                        else if (b.id === 'toggle_right')
                            this.ui.showRightHemisphere = !this.ui.showRightHemisphere;
                        else if (b.id === 'toggle_deep')
                            this.ui.showDeepStructures = !this.ui.showDeepStructures;
                        else if (b.id === 'camera_lock_y')
                            this.interaction.isYLocked = !this.interaction.isYLocked;
                        else if (b.id === 'toggle_baseline')
                            this.ui.isCompareBaseline = !this.ui.isCompareBaseline;
                        else if (b.id === 'sim_pause')
                            this.ui.simPaused = !this.ui.simPaused;
                        else if (b.id === 'show_notes')
                            this.ui.showImplementationNotes = !this.ui.showImplementationNotes;
                    }
                    else if (b.type === 'action') {
                        if (b.id === 'camera_reset')
                            this.resetCamera();
                        else if (b.id === 'export_json')
                            this.exportData('json');
                        else if (b.id === 'export_csv')
                            this.exportData('csv');
                        else if (b.id === 'undo_factor')
                            this.popHistoryState();
                        else if (b.id === 'export_screenshot')
                            this.exportScreenshot();
                        else if (b.id === 'regen_micro') {
                            if (window.GreenhouseInflammationUI3D)
                                window.GreenhouseInflammationUI3D.initMicroData();
                        }
        ...
        handleMouseMove(e) {
            ...
            for (const cat of this.ui.categories) {
                if (mx >= cat.x && mx <= cat.x + cat.w && my >= cat.y && my <= cat.y + cat.h) {
                    this.ui.hoveredElement = Object.assign(Object.assign({}, cat), { type: 'header' });
                    cat.isHovered = true;
                }
                else {
                    cat.isHovered = false;
            ...
            if (!this.ui.hoveredElement) {
                for (const b of this.ui.buttons) {
                    if (mx >= b.x && mx <= b.x + b.w && my >= b.y && my <= b.y + b.h) {
                        this.ui.hoveredElement = Object.assign(Object.assign({}, b), { type: 'button' });
                        break;
                    }
                }
            }
        ...
        handleKeyDown(e) {
            if (e.key === '1')
                this.engine.state.factors.viewMode = 0;
            if (e.key === '2')
                this.engine.state.factors.viewMode = 1;
            if (e.key === '3')
                this.engine.state.factors.viewMode = 2;
            if (e.key === '4')
                this.engine.state.factors.viewMode = 3;
            if (e.key === 'h')
                this.ui.showLeftHemisphere = !this.ui.showLeftHemisphere;
            if (e.key === 'r')
                this.exportData();
        ...
        cycleFocus(dir) {
            ...
            this.ui.categories.forEach(cat => {
                focusables.push(Object.assign(Object.assign({}, cat), { type: 'header' }));
                if (cat.isOpen) {
                    const catBoxes = this.ui.checkboxes.filter(c => c.category === cat.id);
                    catBoxes.forEach(cb => focusables.push(Object.assign(Object.assign({}, cb), { type: 'checkbox' })));
                }
            });
            if (!this.ui.focusedElement) {
        ...
        activateFocusedElement() {
            ...
            if (el.type === 'button') {
                if (el.type === 'toggle') {
                    if (el.id === 'toggle_left')
                        this.ui.showLeftHemisphere = !this.ui.showLeftHemisphere;
                    if (el.id === 'toggle_right')
                        this.ui.showRightHemisphere = !this.ui.showRightHemisphere;
                    if (el.id === 'toggle_deep')
                        this.ui.showDeepStructures = !this.ui.showDeepStructures;
                    if (el.id === 'camera_lock_y')
                        this.interaction.isYLocked = !this.interaction.isYLocked;
                }
                else if (el.type === 'action') {
                    if (el.id === 'camera_reset')
                        this.resetCamera();
                }
                else {
                ...
                this.ui.categories.forEach(c => { if (c.id !== el.id)
                    c.isOpen = false; });
        ...
        updateModel(state, dt) {
            ...
            config.factors.forEach(fact => {
                if (f[fact.id] === 1) {
                    if (fact.category === 'env')
                        scoreEnv++;
                    else if (fact.category === 'psych')
                        scorePsych++;
                    else if (fact.category === 'philo')
                        scorePhilo++;
                    else if (fact.category === 'research')
                        scoreRes++;
                }
            });
        ...
        updateCameraInertia() {
            if (!this.interaction.isDragging) {
                ...
                if (Math.abs(this.interaction.velX) < this.interaction.velocityThreshold)
                    this.interaction.velX = 0;
                if (Math.abs(this.interaction.velY) < this.interaction.velocityThreshold)
                    this.interaction.velY = 0;
        ...
        drawUI(ctx, w, h, state) {
            ...
            const l = this.layout;
            ctx.fillStyle = '#fff';
            ctx.font = 'bold 24px Quicksand, sans-serif'; // Increased size (Enhancement #97)
            ctx.fillText('NEUROINFLAMMATION ENGINE', l.title.x, l.title.y);
            ctx.fillStyle = '#A0AEC0';
            ctx.font = 'bold 14px Quicksand, sans-serif'; // Increased size
            ...
            visibleMetrics.forEach((ml, i) => {
                ...
                if (Math.abs(vel) > 0.0001) {
                    ...
                    if (vel > 0) {
                        ...
                        ctx.lineTo(bx + 95, by + 15);
                        ctx.lineTo(bx + 85, by + 15);
                    }
                    else {
                        ctx.moveTo(bx + 90, by + 15);
                        ctx.lineTo(bx + 95, by + 10);
                ...
                ctx.fillText(ml.l, bx + 10, by + 15);
                ctx.fillStyle = '#fff';
                ctx.font = 'bold 12px Quicksand, sans-serif';
                ctx.fillText(ml.v, bx + 10, by + 30);
                ctx.fillStyle = 'rgba(255,255,255,0.3)';
                ctx.font = '8px monospace';
                ctx.fillText(ml.rv, bx + 10, by + 40); // Raw Value (Item 21)
                // Sparkline (Item 27)
            ...
            this.ui.buttons.forEach(b => {
                if (window.GreenhouseInflammationControls) {
                    ...
                    if (b.id === 'toggle_left' && this.ui.showLeftHemisphere)
                        btnState.factors.viewMode = b.val;
                    if (b.id === 'toggle_right' && this.ui.showRightHemisphere)
                        btnState.factors.viewMode = b.val;
                    if (b.id === 'toggle_deep' && this.ui.showDeepStructures)
                        btnState.factors.viewMode = b.val;
            ...
            if (window.GreenhouseInflammationControls) {
                ...
                if (this.ui.showImplementationNotes)
                    this.drawImplementationNotes(ctx);
            }
            if (window.GreenhouseInflammationAnalysis) {
            ...
            if (this.ui.hoveredElement && window.GreenhouseInflammationTooltips && this.ui.hoveredElement.type !== 'header') {
                ...
                if (this.ui.hoveredElement.type === 'checkbox')
                    this.drawImpactPreview(ctx);
            }
        },
        drawExportStatus(ctx) {
            if (!this.ui.exportStatus)
                return;
