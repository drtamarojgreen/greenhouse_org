    ...
    const GreenhouseInflammationPathway = {
        ...
        render(ctx, state, camera, projection, ui3d) {
            ...
            const projectedNodes = nodes.map((node, i) => {
                ...
                return Object.assign(Object.assign({}, node), p);
            });
        ...
        drawPathwayLegend(ctx, projection) {
            ...
            ctx.fillStyle = '#fff';
            ctx.font = '9px monospace';
            ctx.fillText('ACTIVE FACTOR', x + 25, y + 33);
            ctx.fillStyle = '#A0AEC0';
            ctx.beginPath();
            ctx.arc(x + 15, y + 45, 4, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = '#fff';
            ctx.fillText('INACTIVE', x + 25, y + 48);
            ctx.strokeStyle = 'rgba(208, 208, 208, 0.8)';
            ctx.beginPath();
            ctx.moveTo(x + 10, y + 60);
            ctx.lineTo(x + 20, y + 60);
            ctx.stroke();
            ctx.fillStyle = '#fff';
            ctx.fillText('STIMULATORY', x + 25, y + 63);
            ctx.strokeStyle = 'rgba(160, 174, 192, 0.8)';
            ctx.beginPath();
            ctx.moveTo(x + 10, y + 75);
            ctx.lineTo(x + 20, y + 75);
            ctx.stroke();
            ctx.fillStyle = '#fff';
            ctx.fillText('INHIBITORY', x + 25, y + 78);
        },
