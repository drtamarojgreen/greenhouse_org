    ...
    const GreenhouseInflammationAnalysis = {
        ...
        drawMatrix(ctx, w, h, state, layout, isMicro = false) {
            ...
            elements.forEach((el, i) => {
                ...
                regions.forEach((r, j) => {
                    ...
                    if (el === 'IL10')
                        val = state.metrics.il10;
                    if (el === 'MIC')
                        val = state.metrics.microgliaActivation;
                    if (el === 'BBB')
                        val = 1 - state.metrics.bbbIntegrity;
        ...
        drawTimeline(ctx, w, h, state, layout) {
            ...
            if (mx >= x && mx <= x + 140 && my >= y && my <= y + 80) {
                ...
                this.selectedMetrics.forEach((mid, i) => {
                    ...
                    if (val !== undefined)
                        ctx.fillText(`${val.toFixed(2)}`, mx + 5, y + 25 + i * 25);
                });
            }
            ctx.restore();
        },
        drawTrace(ctx, x, y, w, h, color, val, label, history) {
            ...
            if (label === 'TNF' && val > 0.7) {
                ...
                ctx.setLineDash([2, 2]);
                ctx.beginPath();
                ctx.moveTo(x, y - 0.7 * (h - 5));
                ctx.lineTo(x + w, y - 0.7 * (h - 5));
                ctx.stroke();
                ctx.setLineDash([]);
            }
            ctx.globalAlpha = 1.0;
