...
(function () {
    ...
    const GreenhouseInflammationControls = {
        ...
        drawCheckbox(ctx, app, c, state) {
            ...
            if (app.roundRect)
                app.roundRect(ctx, bx, by, boxSize, boxSize, 3, true, true);
            else {
        ...
        drawButton(ctx, app, b, state) {
            ...
            if (b.type === 'toggle') {
                if (b.id === 'toggle_left')
                    isActive = app.ui.showLeftHemisphere;
                else if (b.id === 'toggle_right')
                    isActive = app.ui.showRightHemisphere;
                else if (b.id === 'toggle_deep')
                    isActive = app.ui.showDeepStructures;
                else if (b.id === 'camera_lock_y')
                    isActive = app.interaction.isYLocked;
            }
            else {
            ...
            if (app.roundRect)
                app.roundRect(ctx, b.x, b.y, b.w, b.h, 6, true);
        ...
        drawAtlasLegend(ctx, app, config, layout) {
            ...
            ctx.fillStyle = 'rgba(0,0,0,0.6)';
            if (app.roundRect)
                app.roundRect(ctx, x, y, w, h, 8, true, true);
            else
            ...
            for (const [key, val] of Object.entries(config.atlasLegend || {})) {
                // Item 81: Filtering
                if (viewMode === 1 && (key === 'trp' || key === 'kyn' || key === 'quin'))
                    continue;
                if (viewMode === 2 && (key === 'microglia' || key === 'astrocyte'))
                    continue;
        ...
        drawMiniMap(ctx, app, w, h, layout) {
            ...
            if (app.roundRect)
                app.roundRect(ctx, mx, my, mw, mh, 5, true, true);
