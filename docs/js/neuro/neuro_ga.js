...
(function () {
    ...
    class NeuroGA {
        ...
        nextRand() {
            if (this.randomIdx >= this.randomBuffer.length)
                this.fillRandomBuffer();
            return this.randomBuffer[this.randomIdx++];
        }
        cloneGenome(genome) {
            return {
                neurons: genome.neurons.map(n => (Object.assign({}, n))),
                connections: genome.connections.map(c => (Object.assign({}, c))),
        ...
        setADHDEnhancement(id, active) {
            if (active)
                this.adhdConfig.activeEnhancements.add(id);
            else
                this.adhdConfig.activeEnhancements.delete(id);
            // Immediate effect mapping
            if (id === 2)
                this.adhdConfig.snr = active ? 0.5 : 0.0;
            if (id === 34)
                this.adhdConfig.noiseSmoothing = active ? 0.8 : 0.0; // Mindfulness
            if (id === 46)
                this.adhdConfig.learningRateBoost = active ? 2.0 : 1.0;
            // New conditions
            if (id === 69)
                this.adhdConfig.comtRate = active ? 2.0 : 1.0;
            if (id === 70)
                this.adhdConfig.maoActivity = active ? 2.5 : 1.0;
            if (id === 101)
                this.adhdConfig.snr = active ? 0.9 : 0.0; // Schizophrenia noise
            if (id === 102)
                this.adhdConfig.dopamineDepletion = active ? 0.7 : 0.0; // Parkinson's
            if (id === 103)
                this.adhdConfig.cholinergicLoss = active ? 0.6 : 0.0; // Alzheimer's
            if (id === 104)
                this.adhdConfig.atpLevel = active ? 0.4 : 1.0; // MDD energy drop
            if (id === 106)
                this.adhdConfig.pruningVariance = active ? 0.8 : 0.0; // Autism
            // Expanded Treatments
            if (id === 107)
                this.adhdConfig.snr = active ? 0.2 : this.adhdConfig.snr; // Antipsychotic dampens noise
            if (id === 108)
                this.adhdConfig.dopamineDepletion = active ? 0.1 : this.adhdConfig.dopamineDepletion; // L-DOPA restores
            if (id === 110)
                this.adhdConfig.moodCycle = active ? 0.5 : this.adhdConfig.moodCycle; // Lithium stabilizes
        }
        ...
        createInitialPopulation() {
            ...
            if (this.adhdConfig.activeEnhancements.has(80) || this.adhdConfig.activeEnhancements.has(85)) {
                ...
                if (this.adhdConfig.activeEnhancements.has(85))
                    this.adhdConfig.inflammationLevel = 0.8;
            }
            this.evaluateFitness();
        }
        removeNeuron(genome, index) {
            if (index < 0 || index >= genome.neurons.length)
                return;
        ...
        evaluateFitness() {
            ...
            this.population.forEach(genome => {
                ...
                if (enhancements.has(68)) {
                    ...
                    if (recentStim > 500)
                        responseSensitivity = 0.5; // Diminished response
                }
                ...
                for (let i = 0; i < neurons.length; i++) {
                    ...
                    for (let tIdx = 0; tIdx < targetPoints.length; tIdx++) {
                        ...
                        if (config.userTarget) {
                            ...
                            if (udx * udx + udy * udy + udz * udz < 2500)
                                fitness += 500;
                        }
                    }
                }
                ...
                if (this.adhdConfig.activeEnhancements.has(96)) {
                    ...
                    if (genome.neurons.some(n => n.region === 'parietalLobe'))
                        fitness -= 30;
                }
                ...
                if (this.adhdConfig.activeEnhancements.has(29)) {
                    genome.neurons.forEach(n => {
                        if (n.region === 'pfc')
                            fitness += 10; // Boost PFC specific fitness
                    });
                }
                ...
                if (this.adhdConfig.activeEnhancements.has(58)) {
                    ...
                    genome.neurons.forEach(n => {
                        if (n.region === 'parietalLobe' || n.region === 'temporalLobe')
                            fitness += 5;
                    });
                ...
                if (this.adhdConfig.activeEnhancements.has(83)) {
                    ...
                    genome.connections.forEach(c => {
                        if (genome.neurons[c.from].region === 'temporalLobe')
                            fitness -= 20;
                    });
                }
                ...
                if (this.adhdConfig.activeEnhancements.has(106)) {
                    genome.connections.forEach(c => {
                        ...
                        if (distSq < 2500)
                            fitness += 10; // Reward local
                        else
                            fitness -= 20; // Penalize long-range
                    });
                }
                ...
                if (this.adhdConfig.activeEnhancements.has(99)) {
                    ...
                    if (genome.connections.length > 20)
                        fitness -= (genome.connections.length - 20) * 10;
                }
                ...
                if (this.adhdConfig.activeEnhancements.has(57))
                    cohesionWeight = 10; // Reward over-connectivity
                ...
                for (let i = 0; i < genome.neurons.length; i++) {
                    ...
                    for (let j = i + 1; j < genome.neurons.length; j++) {
                        ...
                        const dy = n1.y - n2.y;
                        const dz = n1.z - n2.z;
                        if (dx * dx + dy * dy + dz * dz < overlapDistSq)
                            fitness -= 50; // Penalty for overlap
                    }
                }
                ...
                if (this.adhdConfig.activeEnhancements.has(68)) {
                    ...
                    if (this.adhdConfig.stimulationHistory.length > 50)
                        this.adhdConfig.stimulationHistory.shift();
                }
        ...
        step() {
            ...
            if (this.adhdConfig.activeEnhancements.has(12)) {
                ...
                if (roll < 0.1)
                    return this.bestGenome; // Dilation
                if (roll > 0.9)
                    this.evaluateFitness(); // Contraction
            }
            ...
            if (this.adhdConfig.activeEnhancements.has(1)) {
                ...
                if (this.nextRand() < 0.05)
                    this.adhdConfig.blinkCooldown = 5;
            }
            ...
            if (this.adhdConfig.activeEnhancements.has(19) || this.adhdConfig.activeEnhancements.has(82)) {
                ...
                if (this.adhdConfig.activeEnhancements.has(82))
                    this.adhdConfig.snr = Math.min(1.0, this.adhdConfig.snr + 0.01);
            }
        ...
        crossover(parentA, parentB) {
            ...
            for (let i = 0; i < maxConnections; i++) {
                if (this.nextRand() < 0.5) {
                    if (parentA.connections[i])
                        childConnections.push(parentA.connections[i]);
                }
                else {
                    if (parentB.connections[i])
                        childConnections.push(parentB.connections[i]);
                }
            }
            const validConnections = childConnections.filter(c => c.from < childNeurons.length && c.to < childNeurons.length);
            return {
        ...
        mutate(genome) {
            ...
            if (this.adhdConfig.activeEnhancements.has(3))
                hyperMultiplier = 2.5;
            // ADHD: Genetic Predisposition (76) & BDNF (36/63)
            let currentMutationRate = this.mutationRate * hyperMultiplier;
            if (this.adhdConfig.activeEnhancements.has(76))
                currentMutationRate *= 1.5;
            if (this.adhdConfig.activeEnhancements.has(36))
                currentMutationRate *= 2.0;
            if (this.adhdConfig.activeEnhancements.has(63))
            ...
            genome.neurons.forEach(n => {
                if (this.adhdConfig.epigeneticLocks.has(n.id))
                    return;
