...
export const GreenhouseNeuroGeometry = {
    ...
    generateSphere(radius: number, segments: number): Geometry {
        const vertices = [];
        const faces = [];
    ...
    generateBox(size: number): Geometry {
        const s = size / 2;
        const vertices = [
    ...
    generateOctahedron(size: number): Geometry {
        const s = size;
        const vertices = [
    ...
    generateMoleculeCluster(type: string, scale: number = 1.0): Geometry {
        const vertices: any[] = [];
        const faces: any[] = [];
        const centers: any[] = [];
    ...
    generateLipidBilayerSegment(width: number, height: number, subdivisions: number): Geometry {
        const vertices: any[] = [];
        const faces: any[] = [];
    ...
    generateGliaMesh(type: string, scale: number = 1.0): Geometry {
        const vertices: any[] = [];
        const faces: any[] = [];
        ...
        for (let i = 0; i < branches; i++) {
            ...
            const offset = vertices.length;
            vertices.push(...tube.vertices.map((v: any) => ({ ...v, color: type === 'astrocyte' ? '#E0E0E0' : '#ff6666' })));
            tube.faces.forEach((f: any) => faces.push([f[0] + offset, f[1] + offset, f[2] + offset]));
        }
        return { vertices, faces };
    },

    inferRegionFromUnitPosition(x: number, y: number, z: number): string | null {
    ...
    getRegionalSurfaceNoise(region: string | undefined, x: number, y: number, z: number): number {
        switch (region) {
            case 'pfc':
    ...
    initializeBrainShell(brainShell: Geometry) {
        // Use realistic brain mesh if available
        if ((window as any).GreenhouseBrainMeshRealistic) {
            const realisticBrain = (window as any).GreenhouseBrainMeshRealistic.generateRealisticBrain();
            brainShell.vertices = realisticBrain.vertices;
            // realisticBrain.faces already has the { indices: [...] } structure
            brainShell.faces = realisticBrain.faces.map((face: any) => ({ ...face, region: null }));
        ...
        for (let lat = 0; lat <= latitudeBands; lat++) {
            ...
            for (let lon = 0; lon <= longitudeBands; lon++) {
                ...
                if (y < -0.3 && z < -0.4) {
                    const distBack = Math.sqrt(x * x + (y + 0.5) * (y + 0.5) + (z + 0.6) * (z + 0.6));
                    if (distBack < 0.5) {
                        const bulge = 1 + (0.5 - distBack) * 0.6; // Reduced bulge
                        x *= bulge;
                        y *= bulge;
                        z *= bulge;
                    }
    ...
    updateMesh(geometry: Geometry, time: number, state: any = {}) {
    ...
    getRegionVertices(brainShell: Geometry, regionKey: string) {
    ...
    computeRegionsAndBoundaries(brainShell: Geometry) {
        ...
        brainShell.vertices.forEach((v, i) => {
            ...
            for (const [name, data] of Object.entries(brainShell.regions || {})) {
                if ((data as any).vertices && (data as any).vertices.includes(i)) {
                    v.region = name;
                    break;
                }
            }
    ...
    generateTubeMesh(p1: Greenhouse.Point3D, p2: Greenhouse.Point3D, cp: Greenhouse.Point3D, radius: number, segments: number): Geometry {
        const vertices = [];
        const faces = [];
    ...
    createSynapseGeometry(radius: number, segments: number, type: string): Geometry {
        const vertices = [];
        const faces = [];
...
(window as any).GreenhouseNeuroGeometry = GreenhouseNeuroGeometry;
