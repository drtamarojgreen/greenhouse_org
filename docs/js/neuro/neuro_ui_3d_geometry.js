...
const GreenhouseNeuroGeometry = {
    ...
    generateGliaMesh(type, scale = 1.0) {
        ...
        vertices.push(...soma.vertices.map(v => (Object.assign(Object.assign({}, v), { color: type === 'astrocyte' ? '#E0E0E0' : '#ff4444' }))));
        ...
        for (let i = 0; i < branches; i++) {
            ...
            const offset = vertices.length;
            vertices.push(...tube.vertices.map((v) => (Object.assign(Object.assign({}, v), { color: type === 'astrocyte' ? '#E0E0E0' : '#ff6666' }))));
    ...
    inferRegionFromUnitPosition(x, y, z) {
        ...
        if (y > 0.4 && z > -0.4 && z < 0.4)
            return 'parietalLobe';
        if (Math.abs(x) > 0.4 && y < 0.1 && z > -0.4 && z < 0.4)
            return 'temporalLobe';
        if (Math.abs(x) < 0.3 && Math.abs(y) < 0.3 && Math.abs(z) < 0.3)
            return 'amygdala';
        if (Math.abs(x) > 0.2 && Math.abs(x) < 0.5 && y < 0 && z > -0.2 && z < 0.2)
            return 'hippocampus';
        return null;
    },
        ...
        for (let lat = 0; lat <= latitudeBands; lat++) {
            ...
            for (let lon = 0; lon <= longitudeBands; lon++) {
                ...
                if (y < -0.3 && z < -0.4) { // Cerebellum area
                    freq = 40;
                    amp = 0.015; // Denser, finer folds
                }
                else if (z > 0.4) { // PFC area
                    freq = 15;
                    amp = 0.04; // Deeper, more pronounced
                }
                else if (Math.abs(x) > 0.6) { // Temporal
                    freq = 8;
                    amp = 0.025; // Wider, smoother
                }
    ...
    updateMesh(geometry, time, state = {}) {
        ...
        if (!geometry || !Array.isArray(geometry.vertices))
            return geometry;
        const load = Math.max(0, Math.min(1, (_b = (_a = state.cognitiveLoad) !== null && _a !== void 0 ? _a : state.load) !== null && _b !== void 0 ? _b : 0));
        const tension = Math.max(0, Math.min(1, (_c = state.tension) !== null && _c !== void 0 ? _c : 0));
    ...
    getRegionVertices(brainShell, regionKey) {
        const indices = [];
        if (!brainShell || !brainShell.vertices)
            return indices;
    ...
    computeRegionsAndBoundaries(brainShell) {
        ...
        brainShell.vertices.forEach((v, i) => {
            if (!v)
                return;
            if (v.region)
                return; // Preserve existing data from realistic generator
            v.region = null;
