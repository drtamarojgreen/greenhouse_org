...
(function () {
    ...
    const GreenhouseNeuroControls = {
        drawButton(ctx, app, b, isActive) {
            ...
            ctx.lineWidth = 1;
            if (app.roundRect)
                app.roundRect(ctx, b.x, b.y, b.w, b.h, 6, true, true);
            else {
        ...
        drawSlider(ctx, app, s, value, label) {
            ...
            if (app.roundRect)
                app.roundRect(ctx, s.x, s.y + s.h / 2 - 2, s.w, 4, 2, true);
            // Progress
            const progress = (value - s.min) / (s.max - s.min);
            ctx.fillStyle = '#A0AEC0';
            if (app.roundRect)
                app.roundRect(ctx, s.x, s.y + s.h / 2 - 2, s.w * progress, 4, 2, true);
            // Handle
        ...
        drawPanel(ctx, app, x, y, w, h, title) {
            ...
            if (app.roundRect)
                app.roundRect(ctx, x, y, w, h, 16, true);
            ctx.strokeStyle = 'rgba(255,255,255,0.15)';
            ctx.lineWidth = 1;
            if (app.roundRect)
                app.roundRect(ctx, x, y, w, h, 16, false, true);
            // Title
            if (title) {
                ctx.textAlign = 'left';
        ...
        drawSearchBox(ctx, app, s, query) {
            ...
            if (app.roundRect)
                app.roundRect(ctx, s.x, s.y, s.w, s.h, 6, true, true);
            else {
