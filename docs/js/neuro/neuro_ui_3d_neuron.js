...
const GreenhouseNeuroNeuron = {
    ...
    drawNeuron(ctx, neuron, camera, projection, colorOverride, pulseFreq = 0.005, isHovered = false) {
        ...
        const p = window.GreenhouseModels3DMath.project3DTo2D(neuron.x, neuron.y, neuron.z, camera, projection);
        if (p.scale <= 0)
            return;
        ...
        const now = Date.now();
        const activation = Math.max(0, Math.min(1, (_b = (_a = neuron.activationLevel) !== null && _a !== void 0 ? _a : neuron.activation) !== null && _b !== void 0 ? _b : 0));
        const neuronSeed = String((_c = neuron.id) !== null && _c !== void 0 ? _c : `${neuron.x}_${neuron.y}_${neuron.z}`)
            .split('')
        ...
        const projected = transformedVertices.map(v => window.GreenhouseModels3DMath.project3DTo2D(v.x, v.y, v.z, camera, projection));
        ...
        facesWithDepth.forEach(({ vertices, origVertices, depth }) => {
            ...
            const alpha = window.GreenhouseModels3DMath.applyDepthFog(1, depth);
            const dx1 = v2.x - v1.x;
            const dy1 = v2.y - v1.y;
            const dx2 = v3.x - v1.x;
            const dy2 = v3.y - v1.y;
    ...
    drawLODIcon(ctx, p, neuron, type, colorOverride) {
        const alpha = window.GreenhouseModels3DMath.applyDepthFog(0.6, p.depth);
        const size = 3 * p.scale;
        const colorStr = colorOverride || neuron.baseColor || '#ffffff';
        ctx.fillStyle = colorStr;
        ctx.globalAlpha = alpha;
        if (type === 'pyramidal') {
            // Triangle for pyramidal
            ctx.beginPath();
    ...
    _addSynapticPegs(vertices, faces, positions) {
        positions.forEach(pos => {
            ...
            const size = 1.5;
            // Octahedron peg
            vertices.push({ x: pos.x + size, y: pos.y, z: pos.z }, { x: pos.x - size, y: pos.y, z: pos.z }, { x: pos.x, y: pos.y + size, z: pos.z }, { x: pos.x, y: pos.y - size, z: pos.z }, { x: pos.x, y: pos.y, z: pos.z + size }, { x: pos.x, y: pos.y, z: pos.z - size });
            faces.push([idx + 2, idx, idx + 4], [idx + 2, idx + 4, idx + 1], [idx + 2, idx + 1, idx + 5], [idx + 2, idx + 5, idx], [idx + 3, idx + 4, idx], [idx + 3, idx + 1, idx + 4], [idx + 3, idx + 5, idx + 1], [idx + 3, idx, idx + 5]);
        });
    }
};
