...
const GreenhouseNeuroBrain = {
    ...
    drawBrainShell(ctx, brainShell, camera, projection, width, height, activeGene = null) {
        ...
        for (let i = 0; i < brainShell.faces.length; i++) {
            ...
            if (p1 && p2 && p3 && p1.scale > 0 && p2.scale > 0 && p3.scale > 0) {
                ...
                if (cross < 0) {
                    ...
                    if (!v1 || !v2 || !v3)
                        continue;
                    const normal = window.GreenhouseModels3DMath.calculateFaceNormal(v1, v2, v3);
        ...
        facesToDraw.forEach(f => {
            ...
            const color = window.GreenhouseNeuroLighting.calculateLighting(f.normal, center, camera, material);
            ...
            const fog = window.GreenhouseModels3DMath.applyDepthFog(alphaToUse, f.depth);
    ...
    drawOrientationWidget(ctx, camera, width, height) {
        ...
        axes.forEach(axis => {
            const p = window.GreenhouseModels3DMath.project3DTo2D(axis.x * 30, axis.y * 30, axis.z * 30, { x: 0, y: 0, z: -100, rotationX: camera.rotationX, rotationY: camera.rotationY, rotationZ: camera.rotationZ, fov: 200 }, { width: 0, height: 0, near: 1, far: 1000 });
            ctx.beginPath();
