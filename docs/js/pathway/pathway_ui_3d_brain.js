...
(function () {
    ...
    const GreenhousePathwayBrain = {
        ...
        drawShell(ctx, shell, camera, projection, width, height, activeRegion) {
            if (!shell || !shell.vertices || !shell.faces)
                return;
        ...
        drawInteractionPiP(ctx, w, h, moleculeName, sourceUrl) {
            ...
            if (mapName.includes('serotonin') || mapName.includes('5-ht'))
                color = { r: 224, g: 224, b: 224 };
            if (mapName.includes('glutamate'))
                color = { r: 208, g: 208, b: 208 };
            if (mapName.includes('gaba'))
                color = { r: 160, g: 174, b: 192 };
            if (mapName.includes('cortisol'))
