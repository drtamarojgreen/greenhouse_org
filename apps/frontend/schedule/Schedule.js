...
// Simplified message display functions to avoid problematic $w().collapse/expand calls
function displayError(message, duration = 5000) {
    debugger;
    console.error(`Scheduler Error: ${message}`);
    // In a Velo environment, you might use a custom text element to display errors
    // if direct $w().collapse/expand is not working. For now, console logging.
}

function displaySuccess(message, duration = 3000) {
    debugger;
    console.log(`Scheduler Success: ${message}`);
}

function displayInfo(message, duration = 5000) {
    debugger;
    console.info(`Scheduler Info: ${message}`);
}

// Removed hideElement and showElement as they rely on problematic $w().collapse/expand
// and direct DOM manipulation is not allowed.
// All elements will be assumed to be visible by default or managed by Wix editor.
...
function validateField(field, errorElement) {
    ...
    if (errorElement && errorElement.valid) {
        if (isValid) {
            // errorElement.collapse(); // Removed problematic call
            // For now, just clear text if valid
            errorElement.text = '';
        } else {
            errorElement.text = errorMessage;
            // errorElement.expand(); // Removed problematic call
        }
...
function populateConflictsList(conflicts) {
    ...
    if (conflicts && conflicts.length > 0) {
        ...
        // showElement(schedulerState.uiElements.conflictRepeater); // Removed problematic call
    } else {
        conflictRepeater.data = [{ _id: 'no-conflicts', title: 'No conflicts found.', date: '', time: '', reason: '' }];
        // hideElement(schedulerState.uiElements.conflictRepeater); // Removed problematic call
    }
...
function initializePatientApp() {
    ...
    // setupPatientEventListeners(); // Removed problematic call

    // Removed problematic $w().expand() calls.
    // Assuming these elements are visible by default or managed by Wix editor.
...
function setupPatientEventListeners() {
    ...
    // $w(schedulerState.uiElements.serviceSelect).onChange((event) => { // Removed problematic call
    ...
    // $w(schedulerState.uiElements.therapistDropdown).onChange((event) => { // Removed problematic call
    ...
    // $w(schedulerState.uiElements.calendar).onChange((event) => { // Removed problematic call
    //     const selectedDate = event.target.value;
    ...
    $w(schedulerState.uiElements.proposeAppointmentButton).onClick(async (event) => {
        const patientFormFields = [
            schedulerState.uiElements.patientTitleInput.replace('#', ''), // Adjusted substring
            schedulerState.uiElements.patientDateInput.replace('#', ''), // Adjusted substring
            schedulerState.uiElements.patientTimeInput.replace('#', ''), // Adjusted substring
            schedulerState.uiElements.patientPlatformInput.replace('#', ''), // Adjusted substring
            schedulerState.uiElements.serviceSelect.replace('#', '') // Adjusted substring
        ];
        const errorPrefix = 'error'; // e.g., #errorPatientTitleInput

        if (validateForm(patientFormFields, errorPrefix)) {
            // showLoadingSpinner(true); // Removed problematic call
    ...
    patientFormInputIds.forEach(id => {
        const field = $w(id);
        // Removed substring as #schedulingContainer prefix is removed from uiElements
        const errorEl = $w(`#error${id.replace('#', '')}`); // e.g., #errorPatientTitleInput
        if (field.valid && errorEl.valid) {
            ...
            field.onChange(() => { // Clear error on change
                // if (errorEl.valid) errorEl.collapse(); // Removed problematic call
                if (errorEl.valid) errorEl.text = ''; // Adjusted to clear text
            });
        }
    });
    ...
    try {
        ...
        if (appointments.length === 0) {
            ...
            appointmentsRepeater.onItemReady(($item, itemData, index) => {
                ...
                // $item('#editAppointmentButton').collapse(); // Removed problematic call
                // $item('#deleteAppointmentButton').collapse(); // Removed problematic call
            });
            // appointmentsRepeater.expand(); // Removed problematic call
            return;
        }
...
function resetForm() {
    ...
    $w(schedulerState.uiElements.patientDateInput).value = '';
    ...
    patientFormFields.forEach(fieldId => {
        ...
        if (errorEl.valid) errorEl.text = ''; // Use local hideElement
    });
}
...
function showLoadingSpinner(show) {
    ...
    if (spinner.valid && proposeButton.valid) {
        if (show) {
            // spinner.expand(); // Removed problematic call
            // proposeButton.collapse(); // Removed problematic call
        } else {
            // spinner.collapse(); // Removed problematic call
            // proposeButton.expand(); // Removed problematic call
        }
    } else {
...
async function loadAdminAppointmentData(appointmentId) {
    ...
    try {
        ...
        if (!currentAppointment) {
            ...
            // showElement(schedulerState.uiElements.adminMessageText); // Removed problematic call
            // hideElement(schedulerState.uiElements.adminAppointmentForm); // Removed problematic call
            return;
        }

        // Populate the admin form with data
        populateAdminAppointmentForm(currentAppointment, serviceTypes);
        // showElement(schedulerState.uiElements.adminAppointmentForm); // Removed problematic call
        // hideElement(schedulerState.uiElements.adminMessageText); // Removed problematic call
...
function populateAdminAppointmentForm(appointment, serviceTypes) {
    const adminTitleInput = $w(schedulerState.uiElements.adminTitleInput);
    if (adminTitleInput.valid) adminTitleInput.value = appointment.title || '';

    const adminStartInput = $w(schedulerState.uiElements.adminStartInput);
    if (adminStartInput.valid) adminStartInput.value = appointment.start ? new Date(appointment.start).toISOString().substring(0, 16) : '';

    const adminEndInput = $w(schedulerState.uiElements.adminEndInput);
    if (adminEndInput.valid) adminEndInput.value = appointment.end ? new Date(appointment.end).toISOString().substring(0, 16) : '';

    const adminPlatformInput = $w(schedulerState.uiElements.adminPlatformInput);
    if (adminPlatformInput.valid) adminPlatformInput.value = appointment.platform || '';
    ...
    const adminConfirmedCheckbox = $w(schedulerState.uiElements.adminConfirmedCheckbox);
    if (adminConfirmedCheckbox.valid) adminConfirmedCheckbox.checked = appointment.confirmed || false;

    const adminConflictsTextarea = $w(schedulerState.uiElements.adminConflictsTextarea);
    if (adminConflictsTextarea.valid) adminConflictsTextarea.value = appointment.conflicts ? JSON.stringify(appointment.conflicts, null, 2) : '';

    const adminFirstNameInput = $w(schedulerState.uiElements.adminFirstNameInput);
    if (adminFirstNameInput.valid) adminFirstNameInput.value = appointment.firstName || '';

    const adminLastNameInput = $w(schedulerState.uiElements.adminLastNameInput);
    if (adminLastNameInput.valid) adminLastNameInput.value = appointment.lastName || '';

    const adminContactInfoInput = $w(schedulerState.uiElements.adminContactInfoInput);
    if (adminContactInfoInput.valid) adminContactInfoInput.value = appointment.contactInfo || '';

    const adminAnonymousIdInput = $w(schedulerState.uiElements.adminAnonymousIdInput);
    if (adminAnonymousIdInput.valid) adminAnonymousIdInput.value = appointment.anonymousId || '';
...
function initializeAdminApp() {
    const appointmentId = wixLocation.query.appointmentId;
    ...
    if (appointmentId) {
        ...
        const adminMessageText = $w(schedulerState.uiElements.adminMessageText);
        if (adminMessageText.valid) {
            adminMessageText.text = 'Admin Appointment Form: Provide an appointment ID in the URL (e.g., ?appointmentId=123) to load and edit a specific appointment.';
        }
        // showElement(schedulerState.uiElements.adminMessageText); // Removed problematic call
    ...
    if (adminForm.valid) {
        const adminSaveButton = $w(schedulerState.uiElements.adminSaveButton);
        if (adminSaveButton.valid) {
            adminSaveButton.onClick(async () => {
                const formFields = [
                    schedulerState.uiElements.adminTitleInput.replace('#', ''), // Adjusted substring
                    schedulerState.uiElements.adminStartInput.replace('#', ''), // Adjusted substring
                    schedulerState.uiElements.adminEndInput.replace('#', ''), // Adjusted substring
                    schedulerState.uiElements.adminPlatformInput.replace('#', ''), // Adjusted substring
                    schedulerState.uiElements.adminServiceSelect.replace('#', ''), // Adjusted substring
                    schedulerState.uiElements.adminFirstNameInput.replace('#', ''), // Adjusted substring
                    schedulerState.uiElements.adminLastNameInput.replace('#', ''), // Adjusted substring
                    schedulerState.uiElements.adminContactInfoInput.replace('#', '') // Adjusted substring
                ];
        ...
        const adminDeleteButton = $w(schedulerState.uiElements.adminDeleteButton);
        if (adminDeleteButton.valid) {
            adminDeleteButton.onClick(async () => {
                ...
                if (confirmed) {
                    ...
                    try {
                        ...
                        // hideElement(schedulerState.uiElements.adminAppointmentForm); // Removed problematic call
                        const adminMessageText = $w(schedulerState.uiElements.adminMessageText);
                        if (adminMessageText.valid) adminMessageText.text = 'Appointment has been deleted.';
                        // showElement(schedulerState.uiElements.adminMessageText); // Removed problematic call
        ...
        const adminMessageText = $w(schedulerState.uiElements.adminMessageText);
        if (adminMessageText.valid) {
            adminMessageText.text = 'Admin form components not found. Please ensure they are added to the page.';
        }
        // showElement(schedulerState.uiElements.adminMessageText); // Removed problematic call
    }
}
...
async function initScheduler() {
    debugger;
    initSchedulerCallCount++;
    console.log(`initScheduler has been called ${initSchedulerCallCount} times.`);
    ...
    try {
        ...
        // hideElement(schedulerState.uiElements.patientContainer); // Removed problematic call
        // hideElement(schedulerState.uiElements.dashboardContainer); // Removed problematic call
...
function waitForElement(selector, timeout = 10000) {
    return new Promise((resolve, reject) => {
        ...
        const poller = setInterval(() => {
            ...
            // In Velo, just finding the element isn't enough.
            // You have to make sure it's actually been rendered on the page.
            ...
            if (attempts >= maxAttempts) {
                clearInterval(poller);
                reject(new Error(`Element with selector \"${selector}\" was not found or not rendered within ${timeout}ms.`));
            }
        }, interval);
    });
}
...
$w.onReady(function () {
    // Delay initialization to prevent conflict with React hydration
    // setTimeout(() => {
    //    initScheduler();
    //} , 100);
