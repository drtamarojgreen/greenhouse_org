...
$w.onReady(function () {
    ...
    if (wixWindow.formFactor === "Desktop" || wixWindow.formFactor === "Tablet" || wixWindow.formFactor === "Mobile") {
        initGeneticSimulation();
    }
});

function initGeneticSimulation() {
    // Define the selector where the app will be injected
    const targetSelector = "#genetic-app-container";
    ...
    // (via Custom Element or Embed HTML), and the external script `genetic.js` handles the logic.

    // Example: Expose configuration if needed
    const config = {
        populationSize: 20,
        mutationRate: 0.1,
        targetFitness: 0.95
    ...
    console.log("Genetic Simulation Page Ready");
}
