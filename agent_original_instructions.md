# Agent Aventuro: Mission, Protocols, and Task-Specific Instructions
...
## 4. Task-Specific Steering Instructions (Live Simulator Refactor)

During the Greenhouse Live Simulator refactor task, the user provided the following critical steering instructions to correct the course:
...
*   **Systematic Environment Refresh:** Implement a robust `clearEnvironment()` function to ensure a pristine state between model switches and test runs. This must include:
    ...
    *   Resetting the DOM (clearing the container, removing previous model/test scripts, and clearing results overlays).
*   **Browser-Native Unit Testing:** Pivot to browser-native unit tests that interact directly with the live DOM. Avoid mocks that hide actual production failures.
*   **Dedicated Neuro Validation:** Create a "Run Neuro Test" button that loads the Neuro model and executes a specific browser-native test suite (`test_neuro_browser.js`) to provide verifiable results.
*   **Integrity of Global Context:** Ensure that globals like `GreenhouseADHDData` are correctly mapped and available to the models to prevent TypeErrors in production scripts.
...
*   **Accurate Reporting:** The dashboard must reflect actual browser-native execution results, not simulated or hardcoded "success" messages.
