...
def import_neurons(fbx_path):
    if not os.path.exists(fbx_path):
        ...
        return n1, n2
    # Pass directory and files parameters to avoid Blender 5.0+ AttributeError
    bpy.ops.import_scene.fbx(
        filepath=fbx_path,
        directory=os.path.dirname(fbx_path),
        files=[{"name": os.path.basename(fbx_path)}]
    )
    ...
    bsdf.inputs["Emission Color"].default_value = (0.1, 0.4, 0.2, 1)
...
def create_neurotransmitters():
    bpy.ops.mesh.primitive_uv_sphere_add(radius=0.05, location=(0, 5, 0))
    emitter = bpy.context.object
    emitter.name = "CleftEmitter"
    ...
    p_mat.node_tree.nodes["Principled BSDF"].inputs["Base Color"].default_value = (1, 0.8, 0.2, 1)
    p_mat.node_tree.nodes["Principled BSDF"].inputs["Emission Color"].default_value = (1, 0.5, 0, 1)
    p_mat.node_tree.nodes["Principled BSDF"].inputs["Emission Strength"].default_value = 5.0
