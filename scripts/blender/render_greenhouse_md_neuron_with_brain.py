...
def create_logo_background(logo_path):
    ...
    bpy.ops.mesh.primitive_plane_add(size=60, location=(0, 25, 0), rotation=(math.pi / 2, 0, math.pi))
...
def create_crossing_spotlights(target_obj):
    for i, start_x in enumerate([-20, 20]):
        bpy.ops.object.light_add(type='SPOT', location=(start_x, -50, 15))
...
def main():
    ...
    scene.camera.data.lens = 20 # Fit size 60 logo at distance 60
