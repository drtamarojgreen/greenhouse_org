...
def create_intertitle(master_instance, text, frame_start, frame_end):
    ...
    style.set_principled_socket(mat, "Emission Strength", 4.0) # Point 142: Capped at 4.0 for visual balance
...
def create_spinning_logo(master_instance, text_content, frame_start, frame_end):
    ...
    style.set_principled_socket(mat, "Emission Strength", 4.0) # Point 142: Capped at 4.0
...
def create_diagnostic_highlight(master_instance, label, location, frame_start, frame_end, color=(1,1,1,1)):
    ...
    style.animate_pulsing_emission(sphere, frame_start, frame_end, base_strength=3.0, pulse_amplitude=1.0)
