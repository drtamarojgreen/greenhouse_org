...
def set_node_input(node, name, value, frame=None):
    ...
    if target:
        # Point 100: Handle FOG_GLOW naming drift in 5.x
        if name.upper() == 'TYPE' and value == 'FOG_GLOW':
            value = 'Fog Glow' # New display name used as ID in some 5.x builds
...
def insert_looping_noise(obj, data_path, index=-1, frame_start=1, frame_end=15000, strength=0.05, scale=10.0, phase=None):
    ...
    for idx in indices:
        ...
        if not fcurve:
            print(f"Warning: Could not access/create fcurve for {anim_target.name} ({full_path}[{idx}])")
            continue
...
def animate_expression_blend(character_name, frame, expression='NEUTRAL', duration=12):
    ...
    if duration > 0:
        ...
        plant_humanoid.animate_expression(armature, frame - duration, expression=None) # Keeps current
