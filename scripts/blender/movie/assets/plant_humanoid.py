...
def create_bark_material(name, color=(0.106, 0.302, 0.118), quality='hero'):
    ...
    node_noise1 = nodes.new(type='ShaderNodeTexNoise'); node_noise1.inputs['Scale'].default_value = 5.0
    node_noise2 = nodes.new(type='ShaderNodeTexNoise'); node_noise2.inputs['Scale'].default_value = 50.0
    node_mix_noise = nodes.new(type='ShaderNodeMixRGB'); node_mix_noise.blend_type = 'MIX'; node_mix_noise.inputs[0].default_value = 0.3
    ...
    node_mix_curv = nodes.new(type='ShaderNodeMixRGB'); node_mix_curv.blend_type = 'OVERLAY'; node_mix_curv.inputs[0].default_value = 0.5
    ...
    node_voronoi = nodes.new(type='ShaderNodeTexVoronoi'); node_voronoi.feature = 'DISTANCE_TO_EDGE'; node_voronoi.inputs['Scale'].default_value = 20.0
    node_bump = nodes.new(type='ShaderNodeBump'); node_bump.inputs['Strength'].default_value = 0.5
...
def create_leaf_material(name, color=(0.522, 0.631, 0.490), quality='hero'):
    ...
    node_mapping = nodes.new(type='ShaderNodeMapping')
    links.new(node_coord.outputs['Generated'], node_mapping.inputs['Vector'])
    node_wave = nodes.new(type='ShaderNodeTexWave'); node_wave.wave_type = 'BANDS'; node_wave.inputs['Scale'].default_value = 10.0
...
def add_tracking_constraint(obj, target, name="TrackTarget"):
    ...
    con = obj.constraints.new(type='DAMPED_TRACK'); con.target = target; con.track_axis = 'TRACK_NEGATIVE_Y'; con.name = name
...
def create_plant_humanoid(name, location, height_scale=1.0, vine_thickness=0.05, seed=None, include_facial_details=True):
    ...
    container = bpy.data.collections.new(name); bpy.context.scene.collection.children.link(container)
    ...
    torso = bpy.context.object; torso.name = f"{name}_Torso"; bpy.ops.object.shade_smooth()
    ...
    head = bpy.context.object; head.name = f"{name}_Head"; bpy.ops.object.shade_smooth()
    ...
    mat_eye = bpy.data.materials.new(name=f"EyeMat_{name}"); mat_eye.use_nodes = True
    ...
    mat_pupil = bpy.data.materials.new(name=f"PupilMat_{name}"); mat_pupil.use_nodes = True
    ...
    for side in [-1, 1]:
        ...
        eye = bpy.context.object; eye.name = f"{name}_Eye_{side_str}"; eye.parent = head; eye.matrix_parent_inverse = head.matrix_world.inverted(); eye.data.materials.append(mat_eye); facial_parts.append(eye)
        if include_facial_details:
            bpy.ops.mesh.primitive_ico_sphere_add(radius=0.015, location=eye_loc + mathutils.Vector((0, -0.035, 0)))
            pupil = bpy.context.object; pupil.name = f"{name}_Pupil_{side_str}"; pupil.parent = eye; pupil.matrix_parent_inverse = eye.matrix_world.inverted(); pupil.data.materials.append(mat_pupil); facial_parts.append(pupil)
            ...
            brow = bpy.context.object; brow.name = f"{name}_Brow_{side_str}"; brow.parent = head; brow.matrix_parent_inverse = head.matrix_world.inverted(); brow.data.materials.append(mat); facial_parts.append(brow)
    if include_facial_details:
        ...
        mouth = bpy.context.object; mouth.name = f"{name}_Mouth"; mouth.scale = (1.5, 0.2, 0.4); mouth.parent = head; mouth.matrix_parent_inverse = head.matrix_world.inverted(); mouth.data.materials.append(mat_eye); facial_parts.append(mouth)
    arm_height = torso_height * 0.9
    l_arm_start = location + mathutils.Vector((0.2, 0, arm_height)); l_arm_end = location + mathutils.Vector((0.8, 0, arm_height - 0.4))
    left_arm = create_vine(l_arm_start, l_arm_end, radius=vine_thickness); left_arm.name = f"{name}_Arm_L"
    l_fingers = create_fingers(l_arm_end, (l_arm_end - l_arm_start).normalized(), radius=vine_thickness)
    for f in l_fingers: f.parent = left_arm; f.matrix_parent_inverse = left_arm.matrix_world.inverted(); container.objects.link(f)
    r_arm_start = location + mathutils.Vector((-0.2, 0, arm_height)); r_arm_end = location + mathutils.Vector((-0.8, 0, arm_height - 0.4))
    right_arm = create_vine(r_arm_start, r_arm_end, radius=vine_thickness); right_arm.name = f"{name}_Arm_R"
    r_fingers = create_fingers(r_arm_end, (r_arm_end - r_arm_start).normalized(), radius=vine_thickness)
    for f in r_fingers: f.parent = right_arm; f.matrix_parent_inverse = right_arm.matrix_world.inverted(); container.objects.link(f)
    left_leg = create_vine(location + mathutils.Vector((0.1, 0, 0.1)), location + mathutils.Vector((0.3, 0, -0.8)), radius=vine_thickness * 1.5); left_leg.name = f"{name}_Leg_L"
    right_leg = create_vine(location + mathutils.Vector((-0.1, 0, 0.1)), location + mathutils.Vector((-0.3, 0, -0.8)), radius=vine_thickness * 1.5); right_leg.name = f"{name}_Leg_R"
    leaf_template = create_leaf_mesh()
    for i in range(int(15 * height_scale)):
        leaf = bpy.data.objects.new(f"{name}_Leaf_{i}", leaf_template.data); container.objects.link(leaf)
        ...
        leaf.rotation_euler = (random.uniform(0, 3.14), random.uniform(0, 3.14), angle); leaf.data.materials.append(leaf_mat); leaf.parent = head; leaf.matrix_parent_inverse = head.matrix_world.inverted()
    main_parts = [head, left_arm, right_arm, left_leg, right_leg]
    for p in main_parts:
        if p.name not in container.objects: container.objects.link(p)
        if p.type == 'CURVE':
            bpy.ops.object.select_all(action='DESELECT'); p.select_set(True); bpy.context.view_layer.objects.active = p; bpy.ops.object.convert(target='MESH')
        p.data.materials.append(mat); p.parent = torso; p.matrix_parent_inverse = torso.matrix_world.inverted()
    for p in facial_parts:
        if p.name not in container.objects: container.objects.link(p)
    if torso.name not in container.objects: container.objects.link(torso)
    torso.data.materials.append(mat)
    if "Herbaceous" in name:
        staff_loc = location + mathutils.Vector((1.0, -0.2, 0.5)); staff = create_vine(staff_loc, staff_loc + mathutils.Vector((0, 0, 1.8)), radius=0.04); staff.name = f"{name}_ReasonStaff"
        staff.parent = torso; staff.matrix_parent_inverse = torso.matrix_world.inverted(); staff.data.materials.append(mat); container.objects.link(staff)
    elif "Arbor" in name:
        for side in [-1, 1]:
            bpy.ops.mesh.primitive_ico_sphere_add(radius=0.15, location=location + mathutils.Vector((side * 0.3, 0, arm_height + 0.1)))
            plate = bpy.context.object; plate.name = f"{name}_ShoulderPlate_{side}"; plate.parent = torso; plate.matrix_parent_inverse = torso.matrix_world.inverted(); plate.data.materials.append(mat); container.objects.link(plate)
    return torso

def animate_walk(torso, frame_start, frame_end, step_height=0.1, cycle_length=48):
    if not torso: return
    name = torso.name.split('_')[0]
    l_leg, r_leg = bpy.data.objects.get(f"{name}_Leg_L"), bpy.data.objects.get(f"{name}_Leg_R")
    l_arm, r_arm = bpy.data.objects.get(f"{name}_Arm_L"), bpy.data.objects.get(f"{name}_Arm_R")
    ...
    for f in range(frame_start, frame_end + 1, 6):
        ...
        if l_leg and r_leg:
            l_leg.rotation_euler[0] = math.sin(phase * math.pi * 2) * math.radians(30); r_leg.rotation_euler[0] = math.sin(phase * math.pi * 2 + math.pi) * math.radians(30)
            l_leg.keyframe_insert(data_path="rotation_euler", index=0, frame=f); r_leg.keyframe_insert(data_path="rotation_euler", index=0, frame=f)
        if l_arm and r_arm:
            l_arm.rotation_euler[0] = math.sin(phase * math.pi * 2 + math.pi) * math.radians(15); r_arm.rotation_euler[0] = math.sin(phase * math.pi * 2) * math.radians(15)
            l_arm.keyframe_insert(data_path="rotation_euler", index=0, frame=f); r_arm.keyframe_insert(data_path="rotation_euler", index=0, frame=f)
...
def animate_talk(torso, frame_start, frame_end):
    ...
    for f in range(frame_start, frame_end + 1, 4):
        mouth.scale.z = random.uniform(0.1, 1.0); mouth.keyframe_insert(data_path="scale", index=2, frame=f)
    mouth.scale.z = 0.4; mouth.keyframe_insert(data_path="scale", index=2, frame=frame_end)
...
def animate_expression(torso, frame, expression='NEUTRAL'):
    ...
    for i, brow in enumerate(brows):
        ...
        if expression == 'ANGRY': brow.rotation_euler[1] = math.radians(110 if i == 0 else 70); brow.location.z = 0.05
        elif expression == 'SURPRISED': brow.rotation_euler[1] = math.radians(90); brow.location.z = 0.08
        else: brow.rotation_euler[1] = math.radians(90); brow.location.z = 0.06
        brow.keyframe_insert(data_path="rotation_euler", index=1, frame=frame); brow.keyframe_insert(data_path="location", index=2, frame=frame)
    for pupil in pupils:
...
def create_flower(location, name="MentalBloom", scale=0.2):
    ...
    core = bpy.context.object; core.name = f"{name}_Core"; container.objects.link(core); bpy.context.scene.collection.objects.unlink(core)
    mat_petal = bpy.data.materials.new(name=f"{name}_MatPetal"); mat_petal.use_nodes = True
    ...
    for i in range(5):
        ...
        p = bpy.context.object; p.name = f"{name}_Petal_{i}"; p.rotation_euler = (math.radians(45), 0, angle)
        p.parent = core; p.matrix_parent_inverse = core.matrix_world.inverted(); p.data.materials.append(mat_petal); container.objects.link(p); bpy.context.scene.collection.objects.unlink(p)
...
def create_inscribed_pillar(location, name="StoicPillar", height=5.0):
    ...
    pillar = bpy.context.object; pillar.name = name
    mat = bpy.data.materials.new(name=f"PillarMat_{name}"); mat.use_nodes = True
    ...
    for i in range(3):
        ...
        band = bpy.context.object; band.name = f"{name}_Band_{i}"; band.parent = pillar; band.matrix_parent_inverse = pillar.matrix_world.inverted(); band.data.materials.append(mat)
    return pillar
...
def create_scroll(location, name="PhilosophicalScroll"):
    ...
    scroll = bpy.context.object; scroll.name = name
    mat = bpy.data.materials.new(name="ScrollMat"); mat.use_nodes = True
...
def create_procedural_bush(location, name="GardenBush", size=1.0):
    ...
    for i in range(25):
        ...
        leaf = bpy.data.objects.new(f"{name}_Leaf_{i}", leaf_template.data); container.objects.link(leaf)
        leaf.location = location + offset; leaf.rotation_euler = (random.uniform(0, 3.14), random.uniform(0, 3.14), random.uniform(0, 3.14))
        leaf.scale = (size, size, size); leaf.data.materials.append(mat)
    return container
