...
class AssetManager:
    ...
    Architecture Kept: Centralized asset management handles scene clearing,
    asset linking, and geometric normalization in a reproducible way.
    """
    ...
    def _get_metrics(self, rig):
        ...
        for obj in bpy.data.objects:
            if obj.type != 'MESH' or obj in meshes: continue
            arm_mod = next((m for m in obj.modifiers if m.type == 'ARMATURE' and m.object == rig), None)
            if arm_mod is not None or obj.parent == rig: meshes.append(obj)
        all_z = []
        for m in meshes:
            m_eval = m.evaluated_get(dg); mw = m_eval.matrix_world
            for corner in m_eval.bound_box: all_z.append((mw @ mathutils.Vector(corner)).z)
