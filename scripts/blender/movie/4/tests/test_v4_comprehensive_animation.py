...
SCENE4_DIR = os.path.dirname(SCRIPT_DIR)
if SCENE4_DIR not in sys.path:
    sys.path.append(SCENE4_DIR)

from assets_v4.plant_humanoid_v4 import create_plant_humanoid_v4
import config

def test_animation_fluidity():
    print("Testing Scene 4 Fluid Animation & Structural Integrity...")
    
    bpy.ops.wm.read_factory_settings(use_empty=True)
    
    # 1. Create Character
    name = "TestAnim"
    armature = create_plant_humanoid_v4(name, (0,0,0))
    ...
    scene = DialogueSceneV4({name: {"rig_name": name}}, [{"speaker_id": name, "start_frame": 1, "end_frame": 30}])
    ...
    if all(features.get(k) for k in ["Eye.L", "Eyelid.Upper.L", "Eyelid.Lower.L"]):
        ...
        u_block = max(0, eyeball_top - u_lid_base)
        l_block = max(0, l_lid_base - eyeball_bot)
