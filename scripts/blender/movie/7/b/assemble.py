...
def run_assembly():
    ...
    for ent_cfg in config.config.get("ensemble.entities", []):
        char = CharacterBuilder.create(ent_cfg["id"], ent_cfg); char.build(manager); char.apply_pose()
        if char.animator: char.animator.apply_action(char.rig, "idle", 1, ent_cfg.get("parameters", {}))
