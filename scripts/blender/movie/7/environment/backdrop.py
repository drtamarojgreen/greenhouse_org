...
class BackdropModeler(Modeler):
    ...
    def _load_v6_background_images(self, m6_root_override=None):
        ...
        m7_root = os.path.dirname(os.path.dirname(__file__))
        m6_root = m6_root_override or config.get("paths.m6_root") or os.path.join(m7_root, "..", "6")
        ...
        for img in images:
            ...
            # Priority 3: M7 assets
            m7_assets = os.path.join(m7_root, "assets", basename)
            if os.path.exists(m7_assets):
                resolved.append(m7_assets)
                continue
