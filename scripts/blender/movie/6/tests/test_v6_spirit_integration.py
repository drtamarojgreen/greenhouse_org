...
class TestV6SpiritIntegration(unittest.TestCase):
    ...
    def test_character_scales(self):
        ...
        if leafy:
            ...
            # Relaxed scale assertions since majestic normalization is disabled
            self.assertGreater(h_leafy, 1.0, "Spirit too small")
    ...
    def test_character_separation_and_grounding(self):
        ...
        positions = {
            "Herb":     mathutils.Vector(config.CHAR_HERBACEOUS_POS),
            "Arbor":    mathutils.Vector(config.CHAR_ARBOR_POS),
    ...
    def test_character_visibility(self):
        ...
        for name in targets:
            ...
            if any(p in name for p in [config.CHAR_HERBACEOUS, config.CHAR_ARBOR]):
                 ...
                 self.assertGreater(rig_loc.xy.length, 0.1, f"Rig for {name} stayed at origin! (Loc: {rig_loc})")
    ...
    def test_feet_to_head_height(self):
        ...
        for name in spirits:
            ...
            # Assertions relaxed because normalization is disabled
            self.assertGreater(height, 0.1, f"{name} too short")
    ...
    def test_pipeline_6a_6b_readiness(self):
        ...
        col_6a = next((col for col in collections if config.COLL_CHARACTERS.lower() in col.name.lower()), None)
        has_6a = col_6a is not None

        col_6b = next((col for col in collections if "env" in col.name.lower() or "environment" in col.name.lower()), None)
        ...
        self.assertTrue(has_6a or has_6b, "ERROR: Neither Characters nor Environment pipeline markers found in collections.")

    def test_scoping_integrity(self):
        coll = bpy.data.collections.get(config.COLL_CHARACTERS)
        self.assertIsNotNone(coll, f"{config.COLL_CHARACTERS} collection missing")
