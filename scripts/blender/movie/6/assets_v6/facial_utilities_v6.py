...
def _build_pupil_disc(
    ...
    """
    Thin disc that sits flush against the eyeball cornea.
    """
...
def _build_teeth(name, armature):
    ...
    bsdf = teeth_mat.node_tree.nodes.get("Principled BSDF") or teeth_mat.node_tree.nodes.new('ShaderNodeBsdfPrincipled')
    bsdf.inputs[0].default_value = (0.9, 0.9, 0.9, 1)
...
def _build_mole(name, armature):
    ...
    bsdf = mole_mat.node_tree.nodes.get("Principled BSDF") or mole_mat.node_tree.nodes.new('ShaderNodeBsdfPrincipled')
    bsdf.inputs[0].default_value = (0.05, 0.03, 0.02, 1)
