...
def animate_light_flicker(light_name, frame_start, frame_end, strength=0.2, seed=None):
    ...
    if not fcurve:
        # Fallback to fcurves.new if action is not Slotted or if ensure fails
        if hasattr(light_obj.data.animation_data.action, "fcurves"):
             fcurve = light_obj.data.animation_data.action.fcurves.find("energy") or \
                      light_obj.data.animation_data.action.fcurves.new("energy")
