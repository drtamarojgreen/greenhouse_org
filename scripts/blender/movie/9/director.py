...
class Director:
    """
    Universal Director for Movie 9.
    Striving for professional-grade camera animation, modularity, and lifelike character presence.
    """
    ...
    def setup_cinematics(self):
        ...
        Feature Kept: The focal target system from Movie 9 is retained to allow for precise
        automated tracking, which is essential for the professional-grade macro shots in Movie 9.
        """
    ...
    def setup_calligraphy(self):
        ...
        Feature Kept: Calligraphy support ensures consistent branding across all movie versions.
        """
        CalligraphyDirector(self.lc_cfg, config.config.total_frames, M9_ROOT).apply()
    ...
    def setup_environment(self, force=False, env_cfg=None):
        ...
        Modular Asset Constraints: Enforces environment-specific asset logic to prevent misplaced features.
        """
        ...
        context = "greenhouse" # Default context for movie 9 main scene
        constraints = config.config.get("context_constraints", {}).get(context, {})
        disallowed = constraints.get("disallowed_assets", [])

        # Filter environment config based on context constraints
        filtered_env = config.config.get("environment", {}).copy()
        for d in disallowed:
            if d in filtered_env:
                print(f"Filtering out disallowed asset '{d}' for context '{context}'")
                del filtered_env[d]

        ExteriorModeler().build_mesh("Env", filtered_env)
        if not bpy.data.objects.get("Chroma") and "rock_path" not in disallowed:
            BackdropModeler().build_mesh("Chroma", config.config.get("chroma", {}))

        if context != "exterior" or "interior_floor" not in disallowed:
            if not bpy.data.objects.get("Interior"):
                InteriorModeler().build_mesh("Interior", config.config.get("interior", {}))

    def set_eyeline_alignment(self, char_a_id, char_b_id):
        """
        Ported from Movie 3: Ensures characters realistically face each other during dialogue.
        Narrative Cohesion: Direct eye contact facilitates the 'Mirroring' psychological process.
        """
        char_a = bpy.data.objects.get(f"{char_a_id}.Rig") or bpy.data.objects.get(char_a_id)
        char_b = bpy.data.objects.get(f"{char_b_id}.Rig") or bpy.data.objects.get(char_b_id)
        if char_a and char_b:
            vec = char_b.location - char_a.location
            char_a.rotation_euler[2] = math.atan2(vec.y, vec.x) + (math.pi / 2)

    def setup_lighting(self):
        """
        Constructs lighting rigs from config.
        Emotional Accuracy: Lighting intensity pulse simulates a 'heartbeat', while key/rim shifts define the mood.
        Anxiety (sharp contrasts, high Rim), Calm (smooth gradients, higher Key).
        """
    ...
    def compose_ensemble(self):
        ...
        Feature Kept: The fan-based positioning provides a balanced visual weight for the
        background ensemble, ported from Movie 9 to ensure environmental richness.
        """
    ...
    def position_protagonists(self):
        """Faces Herbaceous and Arbor toward each other and ensures visibility."""
        ...
        if herb and arbor:
            ...
            self.set_eyeline_alignment("Herbaceous", "Arbor")
            self.set_eyeline_alignment("Arbor", "Herbaceous")

    def apply_storyline(self):
        """
        Executes storyline events defined in movie_config.json.
        Feature Kept: Modular beat-based storyline execution allows for complex narrative timing
        without hardcoding frame numbers into the core logic.
        """
    ...
    def apply_scene_animations(self):
        ...
        if herb:
            ...
            anim_handler.apply_animation(herb, "walk", 3000, duration=total_f - 3000)
        if arbor:
            anim_handler.apply_animation(arbor, "talking", 1, duration=2999)
            anim_handler.apply_animation(arbor, "shake", 300)
            anim_handler.apply_animation(arbor, "walk", 3000, duration=total_f - 3000)
    ...
    def _execute_event(self, event):
        ...
        Feature Kept: The event-dispatch pattern from Movie 9 is maintained for its
        flexibility in handling visibility, altitude, and custom animations across diverse entity types.
        """
